...
/** صف دفتر مع معلومات الجلسة والشحنة المرتبطة — للتحقق قبل النقل */
type TransferRowRecord = {
  ...
  fees_amount_usd: string | number | null;
...
function normalizeReceipt(value: string | null | undefined): string {
...
function isCancelledStatus(status: string | null | undefined, financialStatus: string | null | undefined): boolean {
  const s = String(status ?? '').trim().toUpperCase();
  const f = String(financialStatus ?? '').trim().toUpperCase();
  return s === 'CANCELLED' || f === 'CANCELLED' || f === 'REVERSED';
}

function summarize(rows: TransferRowRecord[]): TransferSummary {
...
export class DailyLedgerTransferService {
  private async loadRows(
    ...
    rowIds: string[],
    forUpdate: boolean,
  ): Promise<TransferRowRecord[]> {
    if (!rowIds.length) return [];
    const lock = forUpdate ? 'for update of r' : '';
    const result = await client.query<TransferRowRecord>(
      `
      select
  ...
  async validateTransfer(
    scope: DataScope,
    input: { rowIds: string[] },
  ): Promise<{ valid: boolean; summary: TransferSummary; warnings: string[]; errors: string[] }> {
  ...
  private collectIssues(
    ...
    for (const row of rows) {
      ...
      if (collect <= 0 && prepaid <= 0 && hawala <= 0 && serviceFee <= 0) {
        errors.push(
          `السطر ${row.row_no} (${row.receipt_no ?? ''}): يجب إدخال التحصيل أو المدفوع مسبقاً قبل النقل`,
        );
      }
  ...
  private async resolveDriverLabel(client: PoolClient, driverId: string | null | undefined): Promise<string | null> {
    if (!driverId) return null;
    const result = await client.query<{ code: string | null; full_name: string }>(
      `select code, full_name from drivers where id = $1::uuid limit 1`,
      [driverId],
    );
  ...
  private async resolveVehicleLabel(client: PoolClient, vehicleId: string | null | undefined): Promise<string | null> {
  ...
  private async findOrCreateTargetSession(
    client: PoolClient,
    params: {
      companyId: string;
      branchId: string;
  ...
  private async sessionSummary(client: PoolClient, sessionId: string): Promise<TransferSummary> {
    const result = await client.query<TransferRowRecord>(
      `
      select
        r.id, r.row_no, r.receipt_no, r.sender_name, r.receiver_name, r.destination,
  ...
  async confirmTransfer(
    scope: DataScope,
    input: { rowIds: string[]; target: TransferTarget; reason: string },
  ): Promise<{
    transferId: string;
    transferNo: string;
    ...
    targetSummary: TransferSummary;
    sourceSummaries: Array<{ sessionId: string; summary: TransferSummary }>;
  }> {
    if (!scope.companyId) throw new HttpError(400, 'Company scope is required.');

    const reason = String(input.reason ?? '').trim();
    if (!reason) throw new HttpError(400, 'يجب إدخال سبب النقل');

    const target = input.target;
    if (!target?.ledgerDate) throw new HttpError(400, 'يجب تحديد تاريخ الإرسالية الجديدة');
    ...
    try {
      ...
      if (movingReceipts.length) {
        ...
        if (dup.rows[0]) {
          throw new HttpError(
            409,
            `رقم الإيصال «${dup.rows[0].receipt_no}» موجود مسبقاً في الإرسالية الهدف (سطر ${dup.rows[0].row_no})`,
          );
        }
      }

      // رقم البداية لإعادة الترقيم في الجلسة الهدف
      const maxRowNoResult = await client.query<{ max_no: number | null }>(
      ...
      const transferRecord = await client.query<{ id: string }>(
        `
