...
const hostnameRegex =
  /^(localhost|(([a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)\.)*[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?|(\d{1,3}\.){3}\d{1,3})$/;
...
type SupportedSettingKey = (typeof supportedKeys)[number];

const protectedKeys = new Set<SupportedSettingKey>(['runtime.environment']);

const defaultValues: Record<SupportedSettingKey, unknown> = {
...
const validators: Record<SupportedSettingKey, (input: unknown) => unknown> = {
  'network.mode': (input) =>
    z.enum(['local_only', 'lan_branch', 'cloud_ready', 'hybrid_ready']).parse(input),
  'network.host': (input) => {
  ...
  'runtime.environment': (input) =>
    z.enum(['development', 'staging', 'production', 'test']).parse(input),
...
export class SystemSettingsService {
  constructor(private readonly repository = new SystemSettingsRepository()) {}

  private validateSettingKey(key: string): SupportedSettingKey {
    return settingKeySchema.parse(key);
  }

  private validateValue(key: SupportedSettingKey, value: unknown): unknown {
    return validators[key](value);
  }

  private validateNetworkTopology(network: {
    mode: unknown;
    host: unknown;
    port: unknown;
    protocol: unknown;
  ...
  async listSettings(companyId: string): Promise<Record<SupportedSettingKey, unknown>> {
    const rows = await this.repository.listSettings(companyId);
    const merged = { ...defaultValues } as Record<SupportedSettingKey, unknown>;
    for (const row of rows) {
      if (!supportedKeys.includes(row.key as SupportedSettingKey)) continue;
      const key = row.key as SupportedSettingKey;
      try {
  ...
  async getSetting(companyId: string, key: string): Promise<unknown> {
  ...
  async setSetting(companyId: string, key: string, value: unknown, isEncrypted = false) {
    const normalizedKey = this.validateSettingKey(key);
  ...
  async getNetworkConfig(companyId: string) {
  ...
  async getRuntimeConfig(companyId: string) {
    const all = await this.listSettings(companyId);
    return {
  ...
  async getElectronRuntimeConfig(companyId: string) {
    const all = await this.listSettings(companyId);
    return {
