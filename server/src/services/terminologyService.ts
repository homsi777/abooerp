...
export class TerminologyService {
  constructor(private readonly repository = new TerminologyRepository()) {}

  async get(companyId: string): Promise<Record<string, string>> {
  ...
  async update(companyId: string, payload: unknown, userId: string | null) {
    ...
    const sanitized: Record<string, string> = { ...defaultTerminology };
    for (const [key, value] of Object.entries(parsed)) {
      if (!(key in defaultTerminology)) continue;
