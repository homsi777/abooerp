...
function toUsd(amount: number, currency: string, rate: number): number {
  if (currency.toUpperCase() === 'USD') return money(amount);
  return money(amount * (rate || 1));
}

function buildShipmentScope(scope: DataScope | undefined, values: unknown[], alias = 's') {
...
export class ProfitLossReportService {
  async buildReport(scope: DataScope | undefined, filters: ProfitLossReportFilters): Promise<ProfitLossReport> {
    const reportCurrency = filters.currencyCode?.toUpperCase() ?? 'USD';

    const shipmentValues: unknown[] = [filters.fromAt, filters.toAt];
    ...
    const commissionLines: ProfitLossLine[] = [];
    ...
    const transferValues: unknown[] = [filters.fromAt, filters.toAt];
    ...
    const expenseValues: unknown[] = [filters.fromAt, filters.toAt];
    ...
    const expenseLines: ProfitLossLine[] = expenseRows.rows.map((row) => {
      const cur = String(row.original_currency ?? 'USD');
      const amount = money(row.original_amount);
      return {
        section: 'operating_expense' as const,
    ...
    const salaryValues: unknown[] = [filters.fromAt, filters.toAt];
    ...
    const salaryLines: ProfitLossLine[] = salaryRows.rows.map((row) => {
    ...
    const liabilityValues: unknown[] = [filters.toAt];
    let companyParam: number | null = null;
    let branchParam: number | null = null;
    let currencyParam: number | null = null;
    ...
    const agentLiabilityLines: ProfitLossLine[] = agentLiabilityRows.rows.map((row) => ({
    ...
    const customerValues: unknown[] = [filters.toAt];
    ...
    const customerLiabilityLines: ProfitLossLine[] = customerLiabilityRows.rows.map((row) => ({
    ...
    const sumUsd = (lines: ProfitLossLine[]) => lines.reduce((s, line) => s + line.amountUsd, 0);
    const sumAmount = (lines: ProfitLossLine[]) => lines.reduce((s, line) => s + line.amount, 0);
