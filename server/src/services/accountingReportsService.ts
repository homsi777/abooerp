...
export type BalanceSheetSection = {
  ...
  label: string;
  total: number;
  lines: Array<{ label: string; amount: number; reference?: string }>;
};
...
function buildBranchScope(scope: DataScope | undefined, values: unknown[], alias: string) {
...
export class AccountingReportsService {
  async getTrialBalance(scope: DataScope | undefined, filters: AccountingReportFilters) {
    const currency = (filters.currencyCode ?? 'USD').toUpperCase();
    const fromAt = filters.fromAt;
    const toAt = filters.toAt ?? filters.asOf;

    const cashboxValues: unknown[] = [currency];
  ...
  async getBalanceSheet(scope: DataScope | undefined, filters: AccountingReportFilters) {
  ...
  async listPeriodClosures(scope: DataScope | undefined, limit = 50) {
  ...
  async closePeriod(
    ...
    if (!scope?.companyId) throw new Error('Company scope required');
  ...
  async isDateInClosedPeriod(companyId: string, dateIso: string, branchId?: string | null): Promise<boolean> {
...
export function buildHawalaReconciliationPackage(full: Record<string, any>) {
  const shipments = (full.shipments ?? []).filter(
    (s: Record<string, unknown>) =>
      money(s.hawala_amount) > 0 || money(s.transfer_service_fee) > 0,
  );
  const transfers = (full.transfers ?? []).filter(
    (t: Record<string, unknown>) => !t.shipment_id || money(t.amount) > 0,
  );

  const hawalaOnShipments = money(
    shipments.reduce((sum: number, s: Record<string, unknown>) => sum + money(s.hawala_amount), 0),
  );
  const hawalaFeesOnShipments = money(
    shipments.reduce((sum: number, s: Record<string, unknown>) => sum + money(s.transfer_service_fee), 0),
  );
  const remittanceOnShipments = shipments.reduce(
    (sum: number, s: Record<string, unknown>) =>
      sum + computeAgentHawalaRemittanceDue({
        hawalaAmount: s.hawala_amount as number,
        transferServiceFee: s.transfer_service_fee as number,
      }),
    0,
  );

  const standaloneTransfers = transfers.filter((t: Record<string, unknown>) => !t.shipment_id);
  const standalonePrincipal = money(
    standaloneTransfers.reduce((sum: number, t: Record<string, unknown>) => sum + money(t.amount), 0),
  );
  const standaloneFees = money(
    standaloneTransfers.reduce((sum: number, t: Record<string, unknown>) => sum + money(t.transfer_service_fee), 0),
  );
  const standaloneRemittance = money(
    standaloneTransfers.reduce((sum: number, t: Record<string, unknown>) => sum + money(t.agent_remittance_due), 0),
  );
...
export function enrichAgentSettlementSummary(full: Record<string, any>) {
  const shipments = full.shipments ?? [];
  const shippingRemittance = money(
    shipments.reduce(
      (sum: number, s: Record<string, unknown>) =>
        sum +
        computeAgentShippingRemittanceDue({
          prepaidAmount: s.prepaid_amount as number,
          freightCharge: s.freight_charge as number,
          transferFee: s.transfer_fee as number,
          agentCommissionAmount: s.agent_commission_amount_snapshot as number,
        }),
      0,
    ),
  );
  const hawalaRemittance = money(
    shipments.reduce(
      (sum: number, s: Record<string, unknown>) =>
        sum +
        computeAgentHawalaRemittanceDue({
  ...
  for (const s of shipments) {
    ...
    const input = {
      prepaidAmount: row.prepaid_amount as number,
      freightCharge: row.freight_charge as number,
      transferFee: row.transfer_fee as number,
      agentCommissionAmount: row.agent_commission_amount_snapshot as number,
    };
...
export function buildAgentMainBranchReconciliationPackage(full: Record<string, any>) {
  ...
  const grossOnAgent = money(
    (settlement.collectionOnAgent ?? 0) + hawalaRemittanceTotal,
  );
  const netRequiredFromAgent = money(Math.max(grossOnAgent - totalCommission, 0));
  ...
  return {
    ...
    mainBranch: {
      ...
      hawalaPrincipal: money(
        (hawalaSummary.hawalaPrincipalOnShipments ?? 0) + (hawalaSummary.standaloneHawalaPrincipal ?? 0),
      ),
      hawalaServiceFees: money(
        (hawalaSummary.hawalaFeesOnShipments ?? 0) + (hawalaSummary.standaloneHawalaFees ?? 0),
      ),
      ...
      prepaidNote:
        'المبلغ المسبق محصّل في الفرع الرئيسي — لا يُطالب الوكيل بتوريده؛ يستحق الوكيل عمولته على هذا الجزء.',
