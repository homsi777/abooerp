...
export class TransfersRepository {
  ...
  async create(payload: TransferPayload, client?: PoolClient) {
    ...
    const query = `
      INSERT INTO transfers (
        company_id, branch_id, agent_id, shipment_id,
    ...
    const values = [
      payload.company_id, payload.branch_id || null, payload.agent_id || null, payload.shipment_id || null,
  ...
  async markCompleted(
    ...
    const { rows } = await client.query(
      ...
      returning *
      `,
      [id, company_id, input.receiptVoucherId ?? null, input.postedCashboxId ?? null, input.postedByUserId ?? null],
    );
    return rows[0] ?? null;
  }
