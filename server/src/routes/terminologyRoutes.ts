...
export function createTerminologyRouter(service: TerminologyService) {
  const router = Router();
  const auditService = new AuditService();

  router.get(
    '/terminology-settings',
    requirePermissions(['settings.terminology.read']),
  ...
  router.put(
    '/terminology-settings',
    requirePermissions(['settings.terminology.write']),
    asyncHandler(async (req, res) => {
      const companyId = requireCompanyId(req);
