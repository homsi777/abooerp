...
export function createPrinterRouter(printerService: PrinterService) {
  const router = Router();
  const auditService = new AuditService();

  router.get(
    '/printers',
    requirePermissions(['settings.printers.read']),
  ...
  router.post(
    '/printers',
    requirePermissions(['settings.printers.write']),
    requireIdempotencyKey('printers.create'),
    asyncHandler(async (req, res) => {
      const companyId = requireCompanyId(req);
  ...
  router.put(
    '/printers/:id',
    requirePermissions(['settings.printers.write']),
    requireIdempotencyKey('printers.update'),
    asyncHandler(async (req, res) => {
      const companyId = requireCompanyId(req);
  ...
  router.post(
    '/printer-routes',
    requirePermissions(['settings.printers.write']),
    requireIdempotencyKey('printer-routes.create'),
    asyncHandler(async (req, res) => {
      const companyId = requireCompanyId(req);
  ...
  router.put(
    '/printer-routes/:id',
    requirePermissions(['settings.printers.write']),
    requireIdempotencyKey('printer-routes.update'),
    asyncHandler(async (req, res) => {
      const companyId = requireCompanyId(req);
  ...
  router.get(
    '/printer-routes/resolve',
    requirePermissions(['settings.printers.read']),
    asyncHandler(async (req, res) => {
      try {
        const companyId = requireCompanyId(req);
        const documentType = String(req.query.documentType ?? '');
        if (!documentType) throw new HttpError(400, 'documentType query is required.');
        const scopedBranchId = (req as any).requestUserContext?.activeBranchId as string | null | undefined;
