...
async function main() {
  ...
  const receipts = await pool.query(
    `select coalesce(sum(original_amount),0) as total from receipt_vouchers where agent_id = $1 and status = 'confirmed'`,
    [agentId],
  );
  ...
  const commission = await pool.query(
    `select coalesce(sum(agent_commission_amount_snapshot),0) as total from shipments where agent_id = $1 and deleted_at is null`,
    [agentId],
  );
