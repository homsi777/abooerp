...
type DashboardPackageResponse = {
  ...
  analytics: {
    ...
    topParties: Array<{ party_id: string }>;
  } | null;
  comparison: {
    delta: {
...
async function request<T>(
  baseUrl: string,
  path: string,
  init?: RequestInit & { auth?: { userId?: string }; idempotencyKey?: string },
...
async function runPhase39DashboardPackageSmoke() {
  ...
  try {
    ...
    const statementAnalyticsRes = await request<DashboardPackageResponse>(
      baseUrl,
      `/api/v1/party-statements/dashboard-package?partyType=sender_receiver&partyId=${partyId}&fromAt=${encodeURIComponent(fromAt)}&toAt=${encodeURIComponent(toAt)}&tabs=statement,analytics&page=1&pageSize=20&topN=3`,
      { auth: { userId: adminUserId } },
    );
    ...
    const comparisonRes = await request<DashboardPackageResponse>(
      baseUrl,
      `/api/v1/party-statements/dashboard-package?partyType=sender_receiver&partyId=${partyId}&tabs=comparison&comparisonFromAt=${encodeURIComponent(comparisonFromAt)}&comparisonToAt=${encodeURIComponent(comparisonToAt)}`,
      { auth: { userId: adminUserId } },
    );
