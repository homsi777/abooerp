...
async function runC8Smoke() {
  ...
  try {
    const admin = await pool.query<{ id: string; company_id: string; branch_id: string | null }>(
    ...
    const readTerms = await request<{ terms: Record<string, string> }>(baseUrl, '/api/v1/terminology-settings', {
      auth: { userId: adminId },
    });
    ...
    const writeTerms = await request<{ terms: Record<string, string> }>(baseUrl, '/api/v1/terminology-settings', {
      method: 'PUT',
      auth: { userId: adminId },
      body: JSON.stringify({
        terms: {
    ...
    const readShipping = await request<{ fields: Array<unknown>; layout: { labelSize: string } }>(
      baseUrl,
      '/api/v1/shipping-label-settings',
      { auth: { userId: adminId } }
    );
    ...
    const updateShipping = await request<{ layout: { labelSize: string } }>(
      baseUrl,
      '/api/v1/shipping-label-settings',
      {
        method: 'PUT',
        auth: { userId: adminId },
        body: JSON.stringify({
          ...(readShipping.body.data as any),
          layout: {
            ...(readShipping.body.data as any).layout,
            labelSize: '100x150',
          },
        }),
      }
    ...
    const printPlan = await request<{ printerRoute: null | { route_scope: string } }>(
      baseUrl,
      '/api/v1/shipping-label-settings/print-plan',
      { auth: { userId: adminId }, scope: { branchId: branchId! } }
    );
