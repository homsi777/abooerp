...
export function createSystemSettingsRouter(systemSettingsService) {
    const router = Router();
    ...
    router.get('/:key', requirePermissions(['settings.system.read']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
        const key = String(req.params.key);
        const value = await systemSettingsService.getSetting(companyId, key);
        res.json({ success: true, data: { key, value } });
    }));
    router.put('/:key', requirePermissions(['settings.system.write']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
        const key = String(req.params.key);
    ...
    router.delete('/:key', requirePermissions(['settings.system.write']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
        const key = String(req.params.key);
