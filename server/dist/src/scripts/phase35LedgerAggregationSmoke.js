...
async function runPhase35LedgerAggregationSmoke() {
    ...
    try {
        ...
        const ledgerAll = await request(baseUrl, `/api/v1/party-statements/ledger?partyType=sender_receiver&partyId=${partyId}&fromAt=${encodeURIComponent(fromAt)}&page=1&pageSize=50`, { auth: { userId: adminUserId } });
        ...
        const ledgerPage1 = await request(baseUrl, `/api/v1/party-statements/ledger?partyType=sender_receiver&partyId=${partyId}&fromAt=${encodeURIComponent(fromAt)}&page=1&pageSize=1`, { auth: { userId: adminUserId } });
        ...
        const ledgerPage2 = await request(baseUrl, `/api/v1/party-statements/ledger?partyType=sender_receiver&partyId=${partyId}&fromAt=${encodeURIComponent(fromAt)}&page=2&pageSize=1`, { auth: { userId: adminUserId } });
        ...
        const summaryRes = await request(baseUrl, `/api/v1/party-statements/currency-summary?partyType=sender_receiver&partyId=${partyId}&fromAt=${encodeURIComponent(fromAt)}`, { auth: { userId: adminUserId } });
