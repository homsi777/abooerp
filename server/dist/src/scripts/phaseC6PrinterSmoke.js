...
async function runPrinterSmoke() {
    ...
    try {
        const adminUser = await pool.query(`select id, company_id, branch_id from users where username = 'admin' limit 1`);
        ...
        const branchCreate = await request(baseUrl, '/api/v1/branches', {
            method: 'POST',
      ...
      `, [adminUserId, branchId, overrideBranchId]);
        const createPrinter = await request(baseUrl, '/api/v1/printers', {
            method: 'POST',
            auth: { userId: adminUserId },
            body: JSON.stringify({
        ...
        const updatePrinter = await request(baseUrl, `/api/v1/printers/${basePrinterId}`, {
            method: 'PUT',
            auth: { userId: adminUserId },
        ...
        const createFallbackRoute = await request(baseUrl, '/api/v1/printer-routes', {
            method: 'POST',
            auth: { userId: adminUserId },
            body: JSON.stringify({
        ...
        const createOverridePrinter = await request(baseUrl, '/api/v1/printers', {
            method: 'POST',
            auth: { userId: adminUserId },
            body: JSON.stringify({
        ...
        const createOverrideRoute = await request(baseUrl, '/api/v1/printer-routes', {
            method: 'POST',
            auth: { userId: adminUserId },
            body: JSON.stringify({
                branch_id: overrideBranchId,
        ...
        const resolveOverride = await request(baseUrl, `/api/v1/printer-routes/resolve?documentType=shipment_label&branchId=${overrideBranchId}`, { auth: { userId: adminUserId } });
        ...
        const resolveFallback = await request(baseUrl, `/api/v1/printer-routes/resolve?documentType=delivery_note&branchId=${branchId}`, { auth: { userId: adminUserId } });
