...
async function runSmoke() {
    ...
    try {
        ...
        const branchesResp = await request(baseUrl, '/api/v1/branches');
        ...
        const shipmentCreate = await request(baseUrl, '/api/v1/shipments', {
            method: 'POST',
            body: JSON.stringify({
        ...
        const shipmentValidTransition = await request(baseUrl, `/api/v1/shipments/${shipmentId}`, {
            method: 'PUT',
            body: JSON.stringify({ status: 'in_transit' }),
        });
        ensure(shipmentValidTransition.status === 200 && shipmentValidTransition.body.success, 'Valid shipment status transition failed');
        ...
        const vehiclesResp = await request(baseUrl, '/api/v1/vehicles');
        const driversResp = await request(baseUrl, '/api/v1/drivers');
        const vehicleId = vehiclesResp.body.data?.[0]?.id;
        const driverId = driversResp.body.data?.[0]?.id;
        const manifestCreate = await request(baseUrl, '/api/v1/manifests', {
            method: 'POST',
            body: JSON.stringify({
        ...
        const deliveryCreate = await request(baseUrl, '/api/v1/deliveries', {
            method: 'POST',
            body: JSON.stringify({
