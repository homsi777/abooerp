...
async function runC9Smoke() {
    ...
    try {
        ...
        const parties = await pool.query(`select id from senders_receivers where status = 'active' order by created_at asc limit 2`);
        ...
        const [s1, s2] = await Promise.all([
            request(baseUrl, '/api/v1/shipments', {
                method: 'POST',
                auth: { userId: adminId },
        ...
        ensure(shipmentSuccessCount === 1 && shipmentConflictCount === 1, 'Shipment idempotency guard failed under concurrent execution');
        const createdShipment = (s1.status === 201 ? s1.body.data : s2.body.data);
        ...
        const [u1, u2] = await Promise.all([
            request(baseUrl, `/api/v1/shipments/${createdShipment.id}`, {
                method: 'PUT',
                auth: { userId: adminId },
                scope: { branchId: branchId },
        ...
        ensure(optimisticSuccessCount === 1 && optimisticConflictCount === 1, 'Shipment optimistic concurrency guard failed');
        ...
        const [r1, r2] = await Promise.all([
            request(baseUrl, '/api/v1/receipt-vouchers', {
                method: 'POST',
                auth: { userId: adminId },
                scope: { branchId: branchId },
        ...
        ensure(receiptSuccessCount === 1 && receiptConflictCount === 1, 'Finance idempotency guard failed under concurrent execution');
        const diagnostics = await request(baseUrl, '/api/v1/system/diagnostics', {
            auth: { userId: adminId },
            scope: { branchId: branchId },
        });
