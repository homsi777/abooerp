...
async function runBackupSmoke() {
    ...
    try {
        const adminRow = await pool.query(`select id, company_id, branch_id from users where username = 'admin' limit 1`);
        ...
        const createdBackup = await request(baseUrl, '/api/v1/backups', {
            method: 'POST',
            auth: { userId: adminId },
            body: JSON.stringify({
                backupType: 'manual',
        ...
        const listBackups = await request(baseUrl, '/api/v1/backups', {
            auth: { userId: adminId },
        });
        ...
        const verifyBackup = await request(baseUrl, `/api/v1/backups/${backupId}/verify`, {
            method: 'POST',
            auth: { userId: adminId },
            body: JSON.stringify({}),
        });
        ...
        const restoreDryRun = await request(baseUrl, `/api/v1/backups/${backupId}/restore`, {
            method: 'POST',
            auth: { userId: adminId },
            body: JSON.stringify({
        ...
        const readinessBefore = await request(baseUrl, `/api/v1/backups/${backupId}/restore-readiness`, { auth: { userId: adminId } });
        ensure(readinessBefore.status === 200 && readinessBefore.body.success, 'Restore readiness endpoint failed');
        ensure(readinessBefore.body.data?.blockers.some((b) => b.code === 'maintenance_mode_required') === true, 'Maintenance-mode blocker should be reported');
        await request(baseUrl, '/api/v1/system-settings/runtime.maintenanceMode', {
            method: 'PUT',
            auth: { userId: adminId },
            body: JSON.stringify({ value: true }),
        });
        const restoreToken = await request(baseUrl, `/api/v1/backups/${backupId}/restore-token`, {
            method: 'POST',
            auth: { userId: adminId },
            body: JSON.stringify({}),
        });
        ...
        const diagnostics = await request(baseUrl, '/api/v1/backup/diagnostics', {
            auth: { userId: adminId },
        });
        ...
        const policyUpdate = await request(baseUrl, '/api/v1/backup-policy', {
            method: 'PUT',
            auth: { userId: adminId },
            body: JSON.stringify({
                autoEnabled: true,
