...
export class ShippingLabelSettingsService {
    repository;
    printerService;
    constructor(repository = new ShippingLabelSettingsRepository(), printerService = new PrinterService()) {
    ...
    async get(companyId) {
        ...
        const merged = {
            ...defaultConfig,
            ...(row.config ?? {}),
            fields: Array.isArray(row.config?.fields) ? row.config.fields : defaultConfig.fields,
            layout: {
    ...
    async getPrintPlan(companyId, branchId) {
