...
function voucherScopeWhere(scope, startIndex = 1, alias = 'v') {
    const values = [];
    const conditions = [];
    const p = `${alias}.`;
    if (scope?.companyId) {
...
export async function buildVoucherReport(scope, voucherType, filters) {
    ...
    const conditions = [...scoped.conditions, `v.status <> 'cancelled'`];
    if (filters.dateFrom) {
    ...
    for (const row of rows) {
        ...
        if (!summaryByCurrency[cur])
            summaryByCurrency[cur] = { count: 0, total: 0 };
...
export async function buildShipmentsByDateReport(scope, filters) {
    const values = [];
...
export async function buildDailyLedgerSummaryReport(companyId, filters) {
    ...
    const grand = days.reduce((acc, day) => {
