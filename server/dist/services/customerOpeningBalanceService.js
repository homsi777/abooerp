...
export async function syncCustomerOpeningBalance(input, client) {
    ...
    const exchangeRateToUsd = await resolveExchangeRateToUsd(exchangeRateRepository, String(input.companyId ?? ''), currencyCode, new Date().toISOString().slice(0, 10));
