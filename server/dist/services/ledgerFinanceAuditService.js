...
export async function buildLedgerFinanceAuditReport(companyId, fromDate) {
    ...
    const movementByDate = new Map(movementTotals.rows.map((row) => [String(row.ledger_date), row]));
    const days = dailySummary.rows.map((row) => {
