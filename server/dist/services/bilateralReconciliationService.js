...
export class BilateralReconciliationService {
    ...
    async getPreview(companyId, agentId, options) {
        ...
        const resolvedPeriodFrom = options?.fromAt
        ...
        const previousBalance = money(lastApproved?.current_balance ?? legacyLast?.balance_amount ?? 0);
    ...
    async approve(companyId, input) {
        const items = this.normalizeItems(input.items);
        const balanceItem = items.find((item) => item.itemType === 'balance');
        const currentBalance = money(input.currentBalance ?? balanceItem?.agentAmount ?? balanceItem?.companyAmount ?? 0);
