...
export class TransfersService {
    ...
    constructor(repo, financeRepository) {
        this.repo = repo;
    ...
    async createTransfer(payload, client) {
        return this.repo.create(payload, client);
    }
    async ensureShipmentLinkedTransfer(input) {
    ...
    async assertAgentBelongsToCompany(client, agentId, companyId) {
        const result = await client.query(`select a.id from agents a join branches b on b.id = a.branch_id where a.id = $1 and b.company_id = $2 and a.is_active = true limit 1`, [agentId, companyId]);
        if (!result.rowCount)
            throw new HttpError(400, 'الوكيل المحدد غير موجود أو غير نشط.');
    }
    async assertCashbox(client, cashboxId, companyId, currency) {
        const result = await client.query(`select id, company_id, currency_code, is_active, agent_id, branch_id from cashboxes where id = $1 limit 1`, [cashboxId]);
        const cashbox = result.rows[0];
        if (!cashbox)
            throw new HttpError(400, 'الصندوق غير موجود.');
        if (String(cashbox.company_id) !== String(companyId))
            throw new HttpError(403, 'الصندوق خارج نطاق الشركة.');
    ...
    async resolveAgentCashbox(client, companyId, agentId, currency) {
        const result = await client.query(`select id from cashboxes where company_id = $1 and agent_id = $2 and is_active = true and upper(currency_code) = upper($3) order by created_at asc limit 1`, [companyId, agentId, currency]);
        const id = result.rows[0]?.id;
        if (!id)
            throw new HttpError(400, 'لا يوجد صندوق نشط للوكيل بعملة الحوالة.');
        return id;
    }
    async insertAgentTransferMovement(client, input) {
        if (input.amount <= 0)
            return;
        await client.query(`
    ...
    async createTransferAndCollect(input) {
        ...
        try {
            ...
            if (!originAgentId)
                throw new HttpError(400, 'يجب تحديد وكيل المصدر لتسجيل قبض الحوالة.');
    ...
    async getAgentPortalTransfer(id, companyId, agentId) {
        return this.repo.getByIdForAgent(id, companyId, agentId);
    }
    async getAgentPortalTransferByShipmentId(shipmentId, companyId, agentId) {
        return this.repo.getByShipmentIdForAgent(shipmentId, companyId, agentId);
    }
    async updateTransferStatus(id, company_id, status, client) {
    ...
    async deleteTransfer(input) {
        ...
        try {
            ...
            const cancelled = await this.repo.markCancelled(input.id, input.companyId, { cancelledByUserId: input.userId ?? null, cancellationReason: input.reason ?? 'حذف إداري' }, client);
            await client.query('commit');
            return cancelled;
        }
        catch (e) {
    ...
    async completeTransfer(input) {
        ...
        try {
            ...
            const completed = await this.repo.markCompleted(transfer.id, input.companyId, { postedCashboxId: input.cashboxId, payoutPaymentVoucherId: String(voucher.id), postedByUserId: input.userId ?? null }, client);
            await client.query('commit');
            return completed;
        }
        catch (error) {
    ...
    async cancelTransfer(input) {
        ...
        try {
            ...
            await client.query(`update party_financial_movements set is_reversal = true, reverse_reason = $2 where reference_type = 'TRANSFER' and reference_id = $1 and is_reversal = false`, [transfer.id, input.reason ?? 'إلغاء الحوالة']);
            const cancelled = await this.repo.markCancelled(transfer.id, input.companyId, { cancelledByUserId: input.userId ?? null, cancellationReason: input.reason ?? null }, client);
            await client.query('commit');
            return cancelled;
        }
        catch (error) {
