...
const AGENT_VOUCHER_TYPES = ['agent_remittance', 'agent_receipt_from_branch'];
function kindFromType(relatedEntityType) {
...
export class AgentPortalVoucherService {
    ...
    isAgentPortalSettlementType(relatedEntityType) {
        return AGENT_VOUCHER_TYPES.includes(relatedEntityType);
    }
    async resolveCashboxes(companyId, agentId, currency) {
    ...
    async createDraftRequest(input) {
        ...
        const relatedEntityType = input.kind === 'receipt_from_branch' ? 'agent_receipt_from_branch' : 'agent_remittance';
        ...
        const agentRes = await pool.query(`select name, code from agents where id = $1::uuid limit 1`, [input.agentId]);
    ...
    async listForAgent(companyId, agentId) {
    ...
    async confirmDraftPaymentVoucher(id, scope, fxContext) {
        ...
        try {
          ...
          `, [id]);
                ...
                if (!pv)
                    throw new HttpError(409, 'تعذر اعتماد السند — ربما تم اعتماده مسبقاً.');
