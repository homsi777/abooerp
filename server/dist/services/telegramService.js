...
export async function fetchTelegramInboxCandidates(botToken, limit = 30) {
    try {
 ...
 * Never throws.
 */
export async function sendActivationNotification(_companyId, payload) {
    try {
 ...
 * Never throws.
 */
export async function sendAgentShipmentNotification(companyId, agentId, payload) {
    try {
        ...
        const agentRow = await pool.query(`select telegram_chat_id from agents where id = $1 limit 1`, [agentId]);
...
export async function sendLinkedPartyShipmentNotifications(companyId, shipmentId) {
    try {
      ...
      `, [shipmentId, companyId]);
        ...
        if (!fallbackToken && !links.some((l) => l.bot_token))
            return;
        const sent = new Set();
        const timestamp = new Date().toLocaleString('ar-SY', { timeZone: 'Asia/Damascus' });
 ...
 */
export async function sendTestMessage(companyId, targetType, targetId) {
    try {
        const timestamp = new Date().toLocaleString('ar-SY', { timeZone: 'Asia/Damascus' });
        if (targetType === 'agent_bot' && targetId) {
            const bot = await repo.getAgentBotById(companyId, targetId);
            if (!bot)
                return { ok: false, error: 'البوت غير موجود' };
            const text = [
            ...
            const result = await sendTelegramMessage(bot.bot_token, bot.chat_id, text);
            if (result.ok)
                await repo.markAgentBotTested(companyId, targetId);
            auditService.logAsync({
                req: null,
...
export async function sendCompanyNotification(companyId, text) {
    try {
        const dbNotifBot = await notifBotRepo.getFirstActive(companyId);
        const botToken = dbNotifBot?.bot_token ?? process.env.TELEGRAM_NOTIFICATION_BOT_TOKEN;
        const chatId = process.env.TELEGRAM_STAFF_NOTIFICATION_CHAT_ID ??
