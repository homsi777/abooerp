 ...
 */
function resolveFinancialParty(shipment, financial) {
    const responsibilityType = financial.financialResponsibilityType ??
        shipment.financial_responsibility_type ??
    ...
    switch (responsibilityType) {
        case 'AGENT': {
            const agentId = financial.financialResponsibilityId ??
                shipment.financial_responsibility_id ??
                shipment.agent_id;
            if (!agentId) {
        ...
        case 'ACCOUNT_CUSTOMER': {
            const customerId = financial.financialResponsibilityId ??
                shipment.financial_responsibility_id ??
        ...
        case 'CUSTOMER': {
            ...
            if (!cid)
                throw new HttpError(400, 'لا يوجد عميل مرتبط بالشحنة.');
...
async function resolvePartyName(client, partyType, partyId) {
...
export class ShipmentFinancialPostingService {
    shipments;
    finance;
    constructor(shipments, finance) {
        this.shipments = shipments;
        this.finance = finance;
    }
    async lockShipmentForUpdate(client, shipmentId, scope) {
     ...
     */
    async ensurePostedFromLifecycle(shipmentId, scope, actorUserId) {
        ...
        try {
            ...
            const storedType = row.financial_responsibility_type;
            const financial = {
                paymentMode: 'UNPAID',
    ...
    async postShipmentConfirmationFinancials(params) {
        ...
        const effectiveResponsibilityType = financial.financialResponsibilityType ??
            row.financial_responsibility_type ??
            (row.agent_id ? 'AGENT' : partyType === 'customer' ? 'ACCOUNT_CUSTOMER' : 'COMPANY_CASH');
        const effectiveResponsibilityId = financial.financialResponsibilityId ??
            row.financial_responsibility_id ??
        ...
        const payerNameSnap = partyType && partyId ? await resolvePartyName(client, partyType, partyId) : '';
        ...
        if (charge > 0 && partyType && partyId) {
            const chargeNotes = effectiveResponsibilityType === 'AGENT'
        ...
        const legacyPayerKind = effectiveResponsibilityType === 'AGENT'
            ? 'AGENT'
            : effectiveResponsibilityType === 'ACCOUNT_CUSTOMER'
                ? 'CUSTOMER'
                : (financial.payerPartyKind ?? row.payer_party_kind ?? null);
        await client.query(`
      update shipments
      set
    ...
    async assertCashboxAllowed(client, cashboxId, scope, shipment, currency) {
    ...
    async recalculateShipmentPaymentStatus(shipmentId) {
      ...
      `, [shipmentId]);
        ...
        if (paid <= 0)
            ...
            paymentStatus = 'PARTIAL';
        let financialStatus = row.financial_status;
    ...
    async reverseShipmentFinancialPosting(shipmentId, reason, scope, userId) {
        const client = await pool.connect();
        try {
            await client.query('begin');
            const row = await this.shipments.lockShipmentForUpdate(client, shipmentId, scope);
        ...
        where id = $1
        `, [shipmentId, reason]);
            await client.query(`update party_financial_movements
         ...
         where shipment_id = $1
           ...
           and is_reversal = false`, [shipmentId, reason]);
            await client.query('commit');
        }
        catch (e) {
            await client.query('rollback');
    ...
    async recordAdditionalPayment(input) {
        ...
        try {
            ...
            const resolvedForPayment = resolveFinancialParty(row, {
                ...
                financialResponsibilityType: row.financial_responsibility_type ?? undefined,
                financialResponsibilityId: row.financial_responsibility_id ?? undefined,
            });
    ...
    async getShipmentFinancialCard(shipmentId, scope) {
        ...
        const vouchers = await pool.query(`select id, voucher_no, status, original_amount, original_currency, cashbox_id, created_at from receipt_vouchers where shipment_id = $1 order by created_at desc`, [shipmentId]);
        return {
