...
if (process.env.NODE_ENV !== 'production' && process.env.LOCK_SERVER_PORT !== '1') {
    ...
    if (fs.existsSync(devPortsPath)) {
        try {
            const raw = JSON.parse(fs.readFileSync(devPortsPath, 'utf-8'));
...
if (parsed.data.NODE_ENV === 'production' &&
    ...
    throw new Error('Refusing to start in production with default AUTH_JWT_SECRET. Set a long random value in the environment.');
}
