...
const WEB_PUBLIC_ORIGINS = new Set(String(process.env.WEB_PUBLIC_ORIGINS ?? '')
    .split(',')
...
app.use(cors({
    origin: (origin, callback) => {
        if (!origin)
            return callback(null, true); // Electron renderer / curl / health checks
        if (STATIC_ALLOWED_ORIGINS.has(origin))
            return callback(null, true);
        if (WEB_PUBLIC_ORIGINS.has(origin))
            return callback(null, true);
...
const dailyLedgerShipmentPostingService = new DailyLedgerShipmentPostingService(dailyLedgerRepository, shipmentService, agentRepository, shipmentFinancialPostingService);
...
app.use('/api/v1', createSalaryRouter(new SalaryRepository(), new ExchangeRateRepository(), employeeRepositorySingleton, financeRepository));
