...
export function createWarehouseRouter(service) {
    const router = Router();
    router.get('/', requirePermissions(['inventory.warehouse.read']), asyncHandler(async (req, res) => {
        const scope = parseDataScope(req);
        const warehouses = await service.list(scope);
        res.json({ success: true, data: warehouses });
    }));
    router.get('/:id', requirePermissions(['inventory.warehouse.read']), asyncHandler(async (req, res) => {
        const scope = parseDataScope(req);
        const warehouse = await service.getById(String(req.params.id), scope);
        res.json({ success: true, data: warehouse });
    }));
    router.post('/', requirePermissions(['inventory.warehouse.write']), asyncHandler(async (req, res) => {
    ...
    router.put('/:id', requirePermissions(['inventory.warehouse.write']), asyncHandler(async (req, res) => {
        const scope = parseDataScope(req);
        const body = warehouseUpdateSchema.parse(req.body);
        const warehouse = await service.update(String(req.params.id), body, scope);
        res.json({ success: true, data: warehouse });
    }));
    router.delete('/:id', requirePermissions(['inventory.warehouse.write']), asyncHandler(async (req, res) => {
        const scope = parseDataScope(req);
