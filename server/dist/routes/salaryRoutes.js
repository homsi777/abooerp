...
function userId(req) {
    return req.requestUserContext?.userId;
}
...
function advanceOutstandingForPayroll(a) {
    if (a.status === 'cancelled' || a.status === 'repaid')
        return 0;
...
function periodFromAdvanceDate(advanceDate) {
...
async function syncAdvanceOutstandingToPendingSalary(repository, exchangeRepo, companyId, before, after) {
...
async function usdDeltaToSalaryDeductionCurrency(exchangeRepo, companyId, deltaUsd, salaryCurrency, periodY, periodM) {
    if (!Number.isFinite(deltaUsd) || deltaUsd === 0)
        return 0;
    const cur = (salaryCurrency ?? 'USD').trim().toUpperCase();
    if (cur === 'USD')
        return Number(deltaUsd.toFixed(2));
...
export function createSalaryRouter(repository, exchangeRateRepository, employeeRepository, financeRepository) {
    const router = Router();
    const auditService = new AuditService();
    // ── Salary Records ─────────────────────────────────────────────────────────
    router.get('/salary-records/summary', requirePermissions(['hr.salaries.read']), asyncHandler(async (req, res) => {
    ...
    router.get('/salary-records', requirePermissions(['hr.salaries.read']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
    ...
    router.get('/salary-records/employee-dossier/:employeeId', requireAnyPermissions(['hr.employees.read', 'hr.salaries.read', 'hr.advances.read']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
    ...
    router.get('/salary-records/:id', requirePermissions(['hr.salaries.read']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
        const data = await repository.getSalaryById(String(req.params.id), companyId);
        if (!data)
            throw new HttpError(404, 'Salary record not found.');
        res.json({ success: true, data });
    }));
    router.get('/salary-records/:id/advance-deductions', requirePermissions(['hr.salaries.read']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
    ...
    router.post('/salary-records', requirePermissions(['hr.salaries.write']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
    ...
    router.put('/salary-records/:id', requirePermissions(['hr.salaries.write']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
    ...
    router.post('/salary-records/:id/pay', requirePermissions(['hr.salaries.write']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
        ...
        const salaryExchangeRateToUsd = currency === 'USD' ? 1 : await resolveExchangeRateToUsd(exchangeRateRepository, companyId, currency, periodDate);
        const data = await repository.paySalary(String(req.params.id), companyId, { cashboxId: body.cashboxId, paidByUserId: userId(req), salaryExchangeRateToUsd }, financeRepository);
    ...
    router.delete('/salary-records/:id', requirePermissions(['hr.salaries.write']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
    ...
    // ── Advances ───────────────────────────────────────────────────────────────
    router.get('/employee-advances', requirePermissions(['hr.advances.read']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
    ...
    router.get('/employee-advances/outstanding-total', requireAnyPermissions(['hr.advances.read', 'hr.salaries.read']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
    ...
    router.get('/advance-usd-quote', requireAnyPermissions(['hr.advances.read', 'hr.advances.write', 'hr.salaries.read', 'hr.salaries.write']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
    ...
    router.get('/employee-advances/:id', requirePermissions(['hr.advances.read']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
        const data = await repository.getAdvanceById(String(req.params.id), companyId);
        if (!data)
            throw new HttpError(404, 'Advance not found.');
        res.json({ success: true, data });
    }));
    router.post('/employee-advances', requirePermissions(['hr.advances.write']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
    ...
    router.put('/employee-advances/:id', requirePermissions(['hr.advances.write']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
    ...
    router.delete('/employee-advances/:id', requirePermissions(['hr.advances.write']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
        const existing = await repository.getAdvanceById(String(req.params.id), companyId);
        if (!existing)
            throw new HttpError(404, 'Advance not found.');
        ...
        if (oldOutUsd > 0) {
            ...
            const delta = await usdDeltaToSalaryDeductionCurrency(exchangeRateRepository, companyId, -oldOutUsd, salCur, y, m);
