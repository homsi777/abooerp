...
export function createShippingLabelSettingsRouter(service) {
    const router = Router();
    const auditService = new AuditService();
    router.get('/shipping-label-settings', requirePermissions(['settings.shippingLabel.read']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
        const data = await service.get(companyId);
        res.json({ success: true, data });
    }));
    router.put('/shipping-label-settings', requirePermissions(['settings.shippingLabel.write']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
    ...
    router.get('/shipping-label-settings/print-plan', requirePermissions(['settings.shippingLabel.read']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
        const branchId = req.requestUserContext?.activeBranchId ?? null;
