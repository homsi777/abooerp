...
export function createManifestRouter(service) {
    ...
    router.get('/loadable-shipments', requirePermissions(['manifests.read']), asyncHandler(async (req, res) => {
    ...
    router.post('/', requirePermissions(['manifests.write']), requireIdempotencyKey('manifests.create'), asyncHandler(async (req, res) => {
        const scope = parseDataScope(req);
        try {
            ...
            emit({ type: 'manifest.updated', companyId: scope.companyId ?? '', branchId: item.branch_id ?? null, entityId: item.id, timestamp: new Date().toISOString(), correlationId: req.correlationId });
    ...
    router.put('/:id', requirePermissions(['manifests.write']), requireIdempotencyKey('manifests.update'), asyncHandler(async (req, res) => {
        const scope = parseDataScope(req);
        try {
