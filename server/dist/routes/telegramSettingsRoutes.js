...
function requireCompanyId(req) {
    const id = req.requestUserContext?.companyId;
    if (!id)
        throw new HttpError(403, 'Company scope required.');
    return id;
}
function requireUserId(req) {
    const id = req.requestUserContext?.userId;
    if (!id)
...
export function createTelegramSettingsRouter(repo) {
    const router = Router();
    const auditService = new AuditService();
    // ── Notification Bot Status (read-only, from .env — no token exposed) ────────
    router.get('/notification-bot-status', requirePermissions(['settings.telegram.read']), asyncHandler(async (_req, res) => {
    ...
    // ── Agent Bots ────────────────────────────────────────────────────────────
    router.get('/agent-bots', requirePermissions(['settings.telegram.read']), asyncHandler(async (req, res) => {
    ...
    router.get('/agent-bots/:id/full', requirePermissions(['settings.telegram.write']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
        const bot = await repo.getAgentBotById(companyId, String(req.params.id));
        if (!bot)
            throw new HttpError(404, 'Agent bot not found.');
        res.json({ success: true, data: bot });
    }));
    router.post('/agent-bots', requirePermissions(['settings.telegram.write']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
    ...
    router.put('/agent-bots/:id', requirePermissions(['settings.telegram.write']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
        const userId = requireUserId(req);
    ...
    router.delete('/agent-bots/:id', requirePermissions(['settings.telegram.write']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
    ...
    router.post('/agent-bots/:id/disable', requirePermissions(['settings.telegram.write']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
        const userId = requireUserId(req);
    ...
    router.post('/agent-bots/:id/test', requirePermissions(['settings.telegram.write']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
