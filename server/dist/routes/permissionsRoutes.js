...
export function createPermissionsRouter() {
    const router = Router();
    router.get('/', requirePermissions(['permissions.view']), asyncHandler(async (_req, res) => {
    ...
    router.get('/overview', requirePermissions(['permissions.view']), asyncHandler(async (req, res) => {
        ...
        const [users, roles, permissions, agents, branches] = await Promise.all([
            ...
            pool.query(`select count(*)::int as count from roles where is_system = true or company_id = $1`, [companyId]),
            pool.query(`select count(*)::int as count from permissions where is_active = true`),
            pool.query(`select count(*)::int as count from agents a join branches b on b.id = a.branch_id where b.company_id = $1`, [companyId]),
            pool.query(`select count(*)::int as count from branches where company_id = $1`, [companyId]),
        ]);
        res.json({
            success: true,
            data: {
