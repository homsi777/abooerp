...
import { RestoreBlockedError } from '../services/backupService.js';
...
export function createBackupRouter(backupService) {
    const router = Router();
    const auditService = new AuditService();
    router.get('/backups', requirePermissions(['settings.backup.read']), asyncHandler(async (req, res) => {
    ...
    router.get('/backups/:id', requirePermissions(['settings.backup.read']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
    ...
    router.post('/backups', requirePermissions(['settings.backup.write']), requireIdempotencyKey('backup.create'), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
    ...
    router.get('/backups/:id/download', requirePermissions(['settings.backup.read']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
    ...
    router.post('/backups/:id/verify', requirePermissions(['settings.backup.write']), requireIdempotencyKey('backup.verify'), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
    ...
    router.post('/backups/:id/restore-token', requirePermissions(['settings.backup.write']), requireIdempotencyKey('backup.restore-token'), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
        const userId = req.requestUserContext?.userId;
    ...
    router.get('/backups/:id/restore-readiness', requirePermissions(['settings.backup.read']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
        const backup = await backupService.getBackupById(String(req.params.id), companyId);
        if (!backup) {
            res.status(404).json({ success: false, error: 'Backup not found.' });
    ...
    router.post('/backups/:id/restore', requirePermissions(['settings.backup.write']), requireIdempotencyKey('backup.restore'), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
        const userId = req.requestUserContext?.userId;
    ...
    router.get('/backup-policy', requirePermissions(['settings.backup.read']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
        const data = await backupService.getBackupPolicy(companyId);
        res.json({ success: true, data });
    }));
    router.put('/backup-policy', requirePermissions(['settings.backup.write']), requireIdempotencyKey('backup.policy.update'), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
    ...
    router.get('/backup/diagnostics', requirePermissions(['settings.backup.read']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
