...
router.get('/:id/financial-summary', requirePermissions(['customers.view', 'customers.account.view']), asyncHandler(async (req, res) => {
    ...
    const customerResult = await pool.query(`select id, name, is_account_customer, default_currency_code from customers where id = $1`, [id]);
...
router.put('/:id', requirePermissions(['customers.manage']), asyncHandler(async (req, res) => {
    ...
    values.push(id);
    const result = await pool.query(`update customers set ${setClauses.join(', ')} where id = $${values.length} returning *`, values);
