...
const optionalUuid = z.preprocess(emptyToUndefined, z.string().uuid({ message: 'معرّف غير صالح' }).optional());
// ── Validation schemas ────────────────────────────────────────────────────────
...
function getCompanyId(req) {
    return req.requestUserContext?.companyId;
}
...
async function generateCustomerCode(companyId) {
    const prefix = 'CUS';
    const result = await pool.query(`select count(*) as cnt from customers where company_id = $1 or company_id is null`, [companyId ?? null]);
    const cnt = parseInt(result.rows[0].cnt, 10) + 1;
    return `${prefix}-${String(cnt).padStart(5, '0')}`;
}
// ── List customers ────────────────────────────────────────────────────────────
router.get('/', requirePermissions(['customers.view']), asyncHandler(async (req, res) => {
    const scope = parseDataScope(req);
    const companyId = getCompanyId(req);
    const userType = getUserType(req);
    const { search, customer_type, is_account_customer, city, branch_id, agent_id, status, page = '1', limit: limitStr = '50', } = req.query;
    ...
    const countResult = await pool.query(`select count(*) as total from customers c where ${where}`, values);
...
router.get('/search', requirePermissions(['customers.view']), asyncHandler(async (req, res) => {
    const scope = parseDataScope(req);
    const companyId = getCompanyId(req);
    const userType = getUserType(req);
    const { q = '' } = req.query;
...
// ── Get single customer ───────────────────────────────────────────────────────
router.get('/:id', requirePermissions(['customers.view']), asyncHandler(async (req, res) => {
    const scope = parseDataScope(req);
    const userType = getUserType(req);
    const { id } = req.params;
    const result = await pool.query(`
...
router.get('/:id/financial-summary', requirePermissions(['customers.view', 'customers.account.view']), asyncHandler(async (req, res) => {
    const id = String(req.params.id);
    if (!uuidRegex.test(id))
        throw new HttpError(400, 'معرّف العميل غير صالح');
    const customerResult = await pool.query(`select id, name, is_account_customer, default_currency_code, opening_balance_amount, opening_balance_side from customers where id = $1`, [id]);
...
router.get('/:id/shipments', requirePermissions(['customers.view', 'shipments.read']), asyncHandler(async (req, res) => {
    const id = String(req.params.id);
    if (!uuidRegex.test(id))
        throw new HttpError(400, 'معرّف العميل غير صالح');
    const { page = '1', limit: limitStr = '20' } = req.query;
    const offset = (Math.max(1, parseInt(page, 10)) - 1) * parseInt(limitStr, 10);
    const result = await pool.query(`
      select
        s.id,
...
// ── Create customer ───────────────────────────────────────────────────────────
router.post('/', requirePermissions(['customers.manage']), asyncHandler(async (req, res) => {
    ...
    const effectiveAgentId = userType === 'agent' && scope.agentId
        ? scope.agentId
        : (body.agent_id ?? null);
...
// ── Update customer ───────────────────────────────────────────────────────────
router.put('/:id', requirePermissions(['customers.manage']), asyncHandler(async (req, res) => {
    const id = String(req.params.id);
    if (!uuidRegex.test(id))
        throw new HttpError(400, 'معرّف العميل غير صالح');
    ...
    try {
        ...
        values.push(id);
        const result = await client.query(`update customers set ${setClauses.join(', ')} where id = $${values.length} returning *`, values);
...
router.patch('/:id/toggle-status', requirePermissions(['customers.manage']), asyncHandler(async (req, res) => {
    const { id } = req.params;
    ...
    const result = await pool.query(`update customers set status = $1, updated_at = now() where id = $2 returning *`, [newStatus, id]);
    res.json({ success: true, data: result.rows[0] });
}));
