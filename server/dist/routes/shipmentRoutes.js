...
import { CANONICAL_SHIPMENT_STATUSES } from '../domain/shipmentStatus.js';
...
export function createShipmentRouter(service) {
    ...
    router.get('/:id/financial-card', requirePermissions(['shipments.read']), asyncHandler(async (req, res) => {
        const scope = parseDataScope(req);
        const data = await service.getShipmentFinancialCard(String(req.params.id), scope);
        res.json({ success: true, data });
    }));
    router.get('/:id/financial-movements', requirePermissions(['shipments.read']), asyncHandler(async (req, res) => {
    ...
    router.get('/:id/status-history', requirePermissions(['shipments.read']), asyncHandler(async (req, res) => {
        const rows = await service.listStatusHistory(String(req.params.id), parseDataScope(req));
        res.json({ success: true, data: rows });
    }));
    router.post('/', requirePermissions(['shipments.write']), licenseGuard('shipment'), requireIdempotencyKey('shipments.create'), asyncHandler(async (req, res) => {
        const scope = parseDataScope(req);
        try {
            ...
            const payload = uc?.userType === 'agent'
                ? {
                    ...parsedRest,
                    createdBy: uc.userId,
            ...
            emit({ type: 'shipment.created', companyId: scope.companyId ?? '', branchId: item.branch_id ?? null, entityId: item.id, timestamp: new Date().toISOString(), correlationId: req.correlationId });
    ...
    router.put('/:id', requirePermissions(['shipments.write']), requireIdempotencyKey('shipments.update'), asyncHandler(async (req, res) => {
        ...
        try {
            ...
            emit({ type: 'shipment.updated', companyId: scope.companyId ?? '', branchId: item.branch_id ?? null, entityId: item.id, timestamp: new Date().toISOString(), correlationId: req.correlationId });
            res.json({ success: true, data: item });
        }
        catch (error) {
            auditService.logAsync({
    ...
    router.post('/:id/confirm', requirePermissions(['shipments.write']), requireIdempotencyKey('shipments.confirm'), asyncHandler(async (req, res) => {
    ...
    router.post('/:id/repost-financials', requirePermissions(['shipments.write']), requireIdempotencyKey('shipments.repost-financials'), asyncHandler(async (req, res) => {
        const scope = parseDataScope(req);
        const uc = req.requestUserContext;
    ...
    router.post('/:id/record-payment', requirePermissions(['shipments.write', 'finance.vouchers.create']), requireIdempotencyKey('shipments.record-payment'), asyncHandler(async (req, res) => {
        const body = recordPaymentSchema.parse(req.body ?? {});
        const scope = parseDataScope(req);
        const uc = req.requestUserContext;
        const updated = await service.recordShipmentPayment(String(req.params.id), {
            amount: body.amount,
    ...
    const registerStatusAction = (actionPath, permissionKey, actionAudit) => {
        router.post(`/:id/${actionPath}`, requirePermissions(['shipments.write']), requireIdempotencyKey(`shipments.${actionPath}`), asyncHandler(async (req, res) => {
            const payload = statusActionPayloadSchema.parse(req.body ?? {});
            const scope = parseDataScope(req);
            const targetStatus = statusForAction(actionPath);
            if (!CANONICAL_SHIPMENT_STATUSES.includes(targetStatus)) {
