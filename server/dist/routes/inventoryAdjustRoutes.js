...
export function createInventoryAdjustRouter(inventoryService) {
    const router = Router();
     ...
     */
    router.post('/inventory/adjust-stock', requirePermissions(['inventory.stock.write']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
        const userId = req.requestUserContext?.userId;
        const body = adjustStockSchema.parse(req.body);
        const movement = await inventoryService.adjustStock(companyId, body.item_id, body.warehouse_id, body.quantity_delta, body.reason, userId);
