...
export function createAgentRouter(repository, financeService) {
    const router = Router();
    const auditService = new AuditService();
    /** Used by دفتر الشحن اليومي — must work for agent users (shipments.read/write), not only settings.agents.read */
    router.get('/lookup-by-destination', requirePermissions(['shipments.read']), asyncHandler(async (req, res) => {
    ...
    router.get('/', requireAnyPermissions(['settings.agents.read', 'shipments.read']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
        const branchIdQuery = req.query.branchId;
        const branchId = typeof branchIdQuery === 'string' ? branchIdQuery : undefined;
        if (branchId) {
    ...
    router.get('/:id/financial-statement', requireAnyPermissions(['settings.agents.read', 'finance.read', 'finance.view']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
    ...
    router.get('/:id/account-statement', requireAnyPermissions(['settings.agents.read', 'finance.read', 'finance.view']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
    ...
    router.post('/:id/reconciliations', requireAnyPermissions(['finance.write', 'finance.vouchers.write', 'settings.agents.write']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
