...
export function createDailyLedgerRouter(service, transferService) {
    ...
    router.post('/print/document', requirePermissions(['shipments.read']), async (req, res) => {
        const scope = parseDataScope(req);
        const bodySchema = z.object({
    ...
    router.get('/print/documents', requireAnyPermissions(['daily_ledger.documentation.read', 'shipments.read']), async (req, res) => {
        const scope = parseDataScope(req);
        const querySchema = z.object({
            branchId: uuid.optional(),
            dateFrom: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional(),
    ...
    router.get('/print/documents/:id', requireAnyPermissions(['daily_ledger.documentation.read', 'shipments.read']), async (req, res) => {
        const scope = parseDataScope(req);
