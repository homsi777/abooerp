...
export function createCurrencyRouter(currencyRepository, exchangeRateRepository) {
    const router = Router();
    const auditService = new AuditService();
    router.get('/', requirePermissions(['settings.currencies.read']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
        const rows = await currencyRepository.listCurrencies(companyId);
        res.json({ success: true, data: rows });
    }));
    router.post('/', requirePermissions(['settings.currencies.write']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
    ...
    router.put('/:id', requirePermissions(['settings.currencies.write']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
    ...
    router.delete('/:id', requirePermissions(['settings.currencies.write']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
    ...
    router.post('/:id/set-base', requirePermissions(['settings.currencies.write']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
