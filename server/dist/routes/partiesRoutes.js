...
const router = Router();
router.get('/smart-search', requirePermissions(['shipments.read']), asyncHandler(async (req, res) => {
    const scope = parseDataScope(req);
    const companyId = req.requestUserContext?.companyId;
    const userType = String(req.requestUserContext?.userType ?? '').toLowerCase();
    const { query: q = '' } = req.query;
    const term = `%${q}%`;
    // ── 1. Quick contacts (senders_receivers) ────────────────────────────────
    const srConditions = ['sr.status = \'active\''];
    const srValues = [term, term];
    ...
    const cusConditions = ['c.status = \'active\''];
    const cusValues = [term, term];
