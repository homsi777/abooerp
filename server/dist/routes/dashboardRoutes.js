...
function buildShipmentScopeWhere(scope, startIndex = 1, alias = 's') {
    const values = [];
...
export function createDashboardRouter() {
    const router = Router();
    router.get('/overview', requireAnyPermissions(['shipments.read', 'shipments.view', 'agent_portal.view', 'finance.read', 'finance.view']), asyncHandler(async (req, res) => {
        ...
        const [shipmentsKpi, statusRows, financeRows, codRows, recentRows, agentRows, branchRows,] = await Promise.all([
            pool.query(`
          select
