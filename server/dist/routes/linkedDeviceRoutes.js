...
async function getDefaultCompanyId() {
    const result = await pool.query(`select id from companies where code = 'COMP-DEFAULT' limit 1`);
    if (!result.rows[0])
        throw new HttpError(500, 'No company configured.');
...
export function createLinkedDeviceRouter(repository) {
    const router = Router();
    ...
    router.post('/register-device', asyncHandler(async (req, res) => {
        ...
        if (isLocal && !device.is_approved && !device.is_blocked) {
            ...
            const adminResult = await pool.query(`select id from users where company_id = $1 order by created_at asc limit 1`, [companyId]);
    ...
    // ── ADMIN: List devices ───────────────────────────────────────────────────
    router.get('/linked-devices', requirePermissions(['settings.devices.read']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
        const data = await repository.listByCompany(companyId);
        res.json({ success: true, data });
    }));
    // ── ADMIN: Approve device ─────────────────────────────────────────────────
    router.post('/linked-devices/:id/approve', requirePermissions(['settings.devices.write']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
    ...
    // ── ADMIN: Block device ───────────────────────────────────────────────────
    router.post('/linked-devices/:id/block', requirePermissions(['settings.devices.write']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
    ...
    // ── ADMIN: Rename device ──────────────────────────────────────────────────
    router.put('/linked-devices/:id/rename', requirePermissions(['settings.devices.write']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
    ...
    // ── ADMIN: Delete device ──────────────────────────────────────────────────
    router.delete('/linked-devices/:id', requirePermissions(['settings.devices.write']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
