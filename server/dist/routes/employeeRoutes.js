...
export function createEmployeeRouter(repository) {
    const router = Router();
    const auditService = new AuditService();
    router.get('/', requirePermissions(['hr.employees.read']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
        const includeInactive = req.query.includeInactive === '1' || req.query.includeInactive === 'true';
        const data = await repository.list(companyId, includeInactive);
        res.json({ success: true, data });
    }));
    router.get('/:id', requirePermissions(['hr.employees.read']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
    ...
    router.post('/', requirePermissions(['hr.employees.write']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
    ...
    router.put('/:id', requirePermissions(['hr.employees.write']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
    ...
    router.delete('/:id', requirePermissions(['hr.employees.write']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
