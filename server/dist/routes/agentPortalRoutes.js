...
import { normalizeShipmentStatus } from '../domain/shipmentStatus.js';
...
function requireAgentPortalContext(req) {
...
function sourceTypeForMobile(sourceType) {
    const labels = {
...
function referenceTypeForMobile(sourceType) {
    const labels = {
...
function transferForMobile(row, currentAgentId) {
...
function shipmentDetailsForMobile(shipment, linkedTransfer, currentAgentId) {
    const breakdown = calculateShipmentFinancialBreakdown(shipment);
    const shippingAmountToCollectOnDelivery = Math.max(breakdown.totalDueOnDelivery - breakdown.hawalaAmount - breakdown.transferServiceFeeAmount, 0);
    return {
        shipmentInfo: {
...
async function workspaceSummaryPayload(service, financeService, req) {
    ...
    try {
        ...
        receiptToday = receipts.filter((r) => String(r.created_at) >= t0).length;
        paymentToday = payments.filter((r) => String(r.created_at) >= t0).length;
    }
    catch {
        receiptToday = -1;
        paymentToday = -1;
...
export function createAgentPortalRouter(service, financeService, transfersService, agents) {
    ...
    router.get('/profile', requirePermissions(['agent_portal.view']), asyncHandler(async (req, res) => {
        ...
        if (agent.branch_id) {
            const br = await pool.query(`select name from branches where id = $1 limit 1`, [agent.branch_id]);
    ...
    router.post('/transfers', requirePermissions(['agent_portal.view']), asyncHandler(async (req, res) => {
        ...
        const transfer = await transfersService.createAgentPortalTransfer({
            ...
            baseCurrency: req.requestUserContext?.baseCurrency,
        });
    ...
    router.get('/workspace-summary', requirePermissions(['agent_portal.view']), asyncHandler(async (req, res) => {
    ...
    router.get('/stats', requirePermissions(['agent_portal.view']), asyncHandler(async (req, res) => {
        const data = await workspaceSummaryPayload(service, financeService, req);
        res.json({ success: true, data });
    }));
    router.get('/shipments', requirePermissions(['agent_portal.view']), asyncHandler(async (req, res) => {
        const scope = parseDataScope(req);
    ...
    router.get('/vouchers', requirePermissions(['agent_portal.view']), asyncHandler(async (req, res) => {
        const { companyId, agentId } = requireAgentPortalContext(req);
        const items = await financeService.agentPortalVouchers().listForAgent(companyId, agentId);
        res.json({ success: true, data: items });
    }));
    router.post('/vouchers', requirePermissions(['agent_portal.view']), asyncHandler(async (req, res) => {
    ...
    router.post('/shipments', requirePermissions(['agent_portal.view']), asyncHandler(async (req, res) => {
        const { companyId, agentId } = requireAgentPortalContext(req);
        const ctx = req.requestUserContext;
        const branchId = ctx?.scope?.branchId;
        const userId = ctx?.userId;
        if (!branchId)
            throw new HttpError(403, 'AGENT_BRANCH_NOT_LINKED');
        ...
        const originalAmount = Math.max(input.freightCharge + input.senderCollectionAmount + input.hawalaAmount +
            input.transferServiceFee + input.additionalCharges - input.prepaidAmount, 0);
    ...
    router.get('/shipments/:id/details', requirePermissions(['agent_portal.view']), asyncHandler(async (req, res) => {
        const { companyId, agentId } = requireAgentPortalContext(req);
    ...
    router.get('/financial-statement', requirePermissions(['agent_portal.view']), asyncHandler(async (req, res) => {
        const { companyId, agentId, currency } = requireAgentPortalContext(req);
    ...
    router.get('/account-statement', requirePermissions(['agent_portal.view']), asyncHandler(async (req, res) => {
        const { companyId, agentId, currency } = requireAgentPortalContext(req);
        ...
        const orderedRows = [...statement.rows].sort((a, b) => {
        ...
        const openingBalance = orderedRows
            .filter((row) => fromTimestamp !== null && Date.parse(String(row.at)) < fromTimestamp)
    ...
    router.get('/transfers', requirePermissions(['agent_portal.view']), asyncHandler(async (req, res) => {
        const { companyId, agentId } = requireAgentPortalContext(req);
    ...
    router.get('/transfers/:id', requirePermissions(['agent_portal.view']), asyncHandler(async (req, res) => {
        const { companyId, agentId } = requireAgentPortalContext(req);
    ...
    router.post('/transfers/:id/complete', requirePermissions(['agent_portal.view']), asyncHandler(async (req, res) => {
        const { companyId, agentId } = requireAgentPortalContext(req);
    ...
    router.post('/shipments/:id/:action', requirePermissions(['agent_portal.status_action']), asyncHandler(async (req, res) => {
        ...
        const updated = await service.transitionStatus({
            ...
            changedBy: req.requestUserContext?.userId,
