...
export function createNotificationBotRouter(repo) {
    const router = Router();
    ...
    router.post('/:id/test', requirePermissions(['settings.system.write']), asyncHandler(async (req, res) => {
        ...
        if (!chatId)
            throw new HttpError(400, 'chat_id مطلوب لاختبار البوت.');
    ...
    router.post('/party-links/bind', requirePermissions(['settings.system.write']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
        const actorUserId = req.requestUserContext?.userId;
        ...
        if (payload.party_type === 'agent') {
            await pool.query(`update agents set telegram_chat_id = $1, updated_at = now() where id = $2`, [payload.chat_id.trim(), payload.party_id]);
        }
        audit.logAsync({
            req,
