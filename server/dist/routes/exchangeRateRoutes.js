...
export function createExchangeRateRouter(exchangeRateRepository, currencyRepository) {
    const router = Router();
    const auditService = new AuditService();
    router.get('/', requirePermissions(['settings.exchangeRates.read']), asyncHandler(async (req, res) => {
    ...
    router.post('/', requirePermissions(['settings.exchangeRates.write']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
    ...
    router.get('/:currencyId/history', requirePermissions(['settings.exchangeRates.read']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
