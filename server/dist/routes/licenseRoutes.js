...
const activateSchema = z.object({
    licenseCode: z.string()
        .trim()
        .toUpperCase()
        .refine((code) => code === 'TEST1' || /^[A-Z0-9]{4}-[A-Z0-9]{4}-[A-Z0-9]{4}-[A-Z0-9]{4}$/.test(code), { message: 'كود التفعيل غير صالح' }),
    machineId: z.string().optional(),
});
function buildQuotaRemaining(license, usage) {
    return {
...
export function createLicenseRouter(repo) {
    const router = Router();
    /**
    ...
    router.post('/activate', asyncHandler(async (req, res) => {
        ...
        if (!companyId)
            throw new HttpError(503, 'No company found in database');
        ...
        const deviceName = req.requestUserContext?.deviceName;
