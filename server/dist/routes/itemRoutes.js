...
export function createItemRouter(service) {
    const router = Router();
    router.get('/', requirePermissions(['inventory.item.read']), asyncHandler(async (req, res) => {
        const scope = parseDataScope(req);
        const items = await service.list(scope);
        res.json({ success: true, data: items });
    }));
    router.get('/:id', requirePermissions(['inventory.item.read']), asyncHandler(async (req, res) => {
        const scope = parseDataScope(req);
        const item = await service.getById(String(req.params.id), scope);
        res.json({ success: true, data: item });
    }));
    router.post('/', requirePermissions(['inventory.item.write']), asyncHandler(async (req, res) => {
    ...
    router.put('/:id', requirePermissions(['inventory.item.write']), asyncHandler(async (req, res) => {
        const scope = parseDataScope(req);
        const body = itemUpdateSchema.parse(req.body);
        const item = await service.update(String(req.params.id), body, scope);
        res.json({ success: true, data: item });
    }));
    router.delete('/:id', requirePermissions(['inventory.item.write']), asyncHandler(async (req, res) => {
        const scope = parseDataScope(req);
