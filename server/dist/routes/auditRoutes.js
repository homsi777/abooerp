...
export function createAuditRouter(service) {
    const router = Router();
    router.get('/', requirePermissions(['settings.audit.read']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
        const filters = filtersSchema.parse(req.query);
        const logs = await service.list(companyId, filters, parseDataScope(req));
        res.json({ success: true, data: logs });
    }));
    router.get('/:id', requirePermissions(['settings.audit.read']), asyncHandler(async (req, res) => {
        const companyId = requireCompanyId(req);
