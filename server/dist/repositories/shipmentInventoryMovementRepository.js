...
export class ShipmentInventoryMovementRepository {
     ...
     */
    async reserveStock(companyId, shipmentId, lines, createdBy) {
        if (!lines.length)
            return [];
        ...
        try {
            ...
            await client.query(`select id from item_stock
         where company_id = $1 and item_id = any($2::uuid[]) and warehouse_id = any($3::uuid[])
         for update`, [companyId, itemIds, warehouseIds]);
            const inserted = [];
            for (const line of lines) {
                // Read current stock
                const stockRes = await client.query(`select id, quantity_on_hand, quantity_reserved
           from item_stock
           where company_id = $1 and item_id = $2 and warehouse_id = $3`, [companyId, line.itemId, line.warehouseId]);
                ...
                if (available < line.quantity) {
                    ...
                    throw new HttpError(409, `STOCK_NOT_AVAILABLE: item ${line.itemId} in warehouse ${line.warehouseId} — ` +
                        `available ${available.toFixed(4)}, requested ${line.quantity}`);
                }
                if (stockRow) {
                    // Upsert increment reserved
                    await client.query(`update item_stock
             set quantity_reserved = quantity_reserved + $1, updated_at = now()
             where company_id = $2 and item_id = $3 and warehouse_id = $4`, [line.quantity, companyId, line.itemId, line.warehouseId]);
                }
                else {
                    // Stock row doesn't exist — treat as zero stock and reject
                    await client.query('rollback');
                    throw new HttpError(409, `STOCK_NOT_AVAILABLE: no stock record for item ${line.itemId} in warehouse ${line.warehouseId}`);
                }
                // Insert movement record
                const mvRes = await client.query(`insert into shipment_inventory_movements
     ...
     */
    async releaseStock(companyId, shipmentId, createdBy) {
        const client = await pool.connect();
        try {
            await client.query('begin');
            // Find all active reservations not yet released or deducted
            const activeRes = await client.query(`select item_id, warehouse_id, sum(quantity) as total_reserved
         from shipment_inventory_movements
         where company_id = $1
           and shipment_id = $2
           and movement_type = 'reserved'
         group by item_id, warehouse_id`, [companyId, shipmentId]);
            const released = [];
           ...
           where company_id = $2 and item_id = $3 and warehouse_id = $4`, [qty, companyId, row.item_id, row.warehouse_id]);
                const mvRes = await client.query(`insert into shipment_inventory_movements
             (company_id, shipment_id, item_id, warehouse_id, quantity, movement_type, notes, created_by)
     ...
     */
    async deductStock(companyId, shipmentId, createdBy) {
        const client = await pool.connect();
        try {
            await client.query('begin');
    ...
    async getShipmentMovements(shipmentId, companyId) {
        const result = await pool.query(`select * from shipment_inventory_movements
       where shipment_id = $1 and company_id = $2
       order by created_at asc`, [shipmentId, companyId]);
        return result.rows;
    }
    async getWarehouseMovements(warehouseId, companyId, limit = 200) {
        const result = await pool.query(`select * from shipment_inventory_movements
       where warehouse_id = $1 and company_id = $2
       order by created_at desc
       limit $3`, [warehouseId, companyId, limit]);
        return result.rows;
    }
    async getItemMovements(itemId, companyId, limit = 200) {
        const result = await pool.query(`select * from shipment_inventory_movements
       where item_id = $1 and company_id = $2
       order by created_at desc
       limit $3`, [itemId, companyId, limit]);
        return result.rows;
    }
    /**
     ...
     */
    async adjustStock(companyId, itemId, warehouseId, quantityDelta, reason, createdBy) {
        if (quantityDelta === 0) {
         ...
         for update`, [companyId, itemId, warehouseId]);
           ...
           on conflict (company_id, warehouse_id, item_id) do update
             set quantity_on_hand = item_stock.quantity_on_hand + $4,
                ...
                if (newOnHand < 0) {
                    throw new HttpError(409, `STOCK_NEGATIVE_VIOLATION: Adjustment of ${quantityDelta} would result in ${newOnHand} on-hand (current: ${currentOnHand}).`);
                }
                await client.query(`update item_stock
           set quantity_on_hand = $1, updated_at = now()
           where id = $2`, [newOnHand, stockRes.rows[0].id]);
            }
            const absQty = Math.abs(quantityDelta);
            const direction = quantityDelta > 0 ? '+' : '-';
            const movementRes = await client.query(`insert into shipment_inventory_movements(
    ...
    async isFinanceCompanyIsolationComplete() {
        try {
            const result = await pool.query(`
