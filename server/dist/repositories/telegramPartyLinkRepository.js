...
export class TelegramPartyLinkRepository {
    ...
    async save(input) {
        ...
        `, [input.companyId, agentId, customerId, senderReceiverId]);
          ...
          `, [
            ...
            if (!row)
                throw new Error('Failed to load saved telegram party link.');
            return row;
        }
        catch (error) {
    ...
    async remove(id, companyId) {
        const result = await pool.query(`delete from telegram_party_links where id = $1 and company_id = $2`, [id, companyId]);
        return (result.rowCount ?? 0) > 0;
    }
    async findForShipment(companyId, shipmentId) {
        const result = await pool.query(`
      with s as (
