...
export class LicenseRepository {
    async findActiveByCompany(companyId) {
        const res = await pool.query(`select * from license_activations
    ...
    async resolveDefaultCompanyId() {
        const res = await pool.query(`select id from companies order by created_at limit 1`);
        return res.rows[0]?.id ?? null;
    }
    async activate(data) {
        // Deactivate any existing licenses for this company
        await pool.query(`update license_activations set is_active = false where company_id = $1`, [data.companyId]);
        const res = await pool.query(`insert into license_activations
    ...
    async getUsage(companyId) {
        ...
        (select count(*) from receipt_vouchers where company_id = $1)::text                       as receipts_used`, [companyId]);
