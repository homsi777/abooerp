...
import { computeAgentBalanceDue, computeCommissionOnPrepaidPortion, computeAgentHawalaRemittanceDue, computeAgentRemittanceDue, computeAgentShippingRemittanceDue, computeAgentTransferRemittanceDue, computeNetRequiredFromAgent, resolveAgentTransferRole, resolvePrepaidAtMainBranch, } from '../utils/agentShipmentSettlement.js';
function shipmentShippingPriceSql(alias = 's') {
    const p = `${alias}.`;
...
function shipmentTableShippingPriceSql() {
...
function resolveAgentReportCurrency(code) {
    const normalized = String(code ?? 'USD').trim().toUpperCase();
    return normalized || 'USD';
}
function shipmentEffectiveAtSql(alias = '') {
    const p = alias ? `${alias}.` : '';
...
export class AgentRepository {
    ...
    async updateAgent(id, companyId, data) {
        ...
        const merged = {
            ...
            branch_id: data.branch_id === undefined || data.branch_id === null
                ? (existing.branch_id ?? '')
                : data.branch_id,
    ...
    async getAgentUsageSummary(companyId, agentId) {
        const result = await pool.query(`
      select
    ...
    async removeAgentPermanently(id, companyId) {
        ...
        if (usage.shipments > 0) {
            throw new HttpError(409, `لا يمكن حذف الوكيل «${agent.code}» لوجود ${usage.shipments} شحنة مرتبطة. يمكنك تعطيله بدلاً من الحذف.`);
        }
        if (usage.users > 0) {
            throw new HttpError(409, `لا يمكن حذف الوكيل «${agent.code}» لوجود ${usage.users} مستخدم مرتبط. يمكنك تعطيله بدلاً من الحذف.`);
        }
        const result = await pool.query(`
    ...
    async assertAgentBusinessRules(companyId, data, excludeAgentId) {
      ...
      `, [companyId, excludeAgentId ?? null, lookupKey]);
        if (conflicts.rows.length > 0) {
            ...
            throw new HttpError(409, `وجهة «${governorate}» مرتبطة بوكيل نشط آخر: ${sample}. يجب وكيل واحد فقط لكل محافظة — عطّل أو احذف المكرر.`);
        }
    }
    async lookupByDestination(companyId, destination, _branchId) {
    ...
    async getLastAgentReconciliationRecord(companyId, agentId) {
    ...
    async createAgentReconciliation(companyId, agentId, input) {
    ...
    async getAgentFinancialStatement(companyId, agentId, options) {
        const opts = typeof options === 'string' ? { currencyCode: options } : options ?? {};
      ...
      limit 500
      `, shipmentValues);
        const transferValues = [companyId, agentId, currencyCode ?? null];
        const transferDateFilters = [];
        if (opts.fromAt) {
      ...
      `, [companyId, agentId, currencyCode ?? null]);
        ...
        const summaryShipmentDateFilters = [];
        const summaryTransferDateFilters = [];
        const summaryVoucherDateFilters = [];
        if (opts.fromAt) {
      ...
      `, [agentId, currencyCode]);
        ...
        return {
            ...
            transfers: transfers.rows.map((row) => {
                const agentRole = resolveAgentTransferRole(agentId, row.origin_agent_id, row.destination_agent_id, row.agent_id);
                return {
                    ...row,
    ...
    async getAgentAccountStatement(companyId, agentId, currencyCode, rowLimit = 1000) {
        const agent = await this.getAgentById(agentId, companyId);
        if (!agent)
            return null;
