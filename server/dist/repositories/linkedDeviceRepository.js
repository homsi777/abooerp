...
export class LinkedDeviceRepository {
    ...
    // ── Heartbeat ────────────────────────────────────────────────────────────────
    async heartbeat(machineId, ipAddress) {
        await pool.query(`
    ...
    async findByMachineId(machineId) {
        const result = await pool.query(`select * from linked_devices where machine_id = $1 limit 1`, [machineId]);
    ...
    // ── Approve ──────────────────────────────────────────────────────────────────
    async approve(id, companyId, approvedBy) {
        const result = await pool.query(`
      update linked_devices
    ...
    // ── Rename ───────────────────────────────────────────────────────────────────
    async rename(id, companyId, name) {
        const result = await pool.query(`
      update linked_devices
    ...
    async remove(id, companyId) {
        const result = await pool.query(`delete from linked_devices where id = $1 and company_id = $2 returning id`, [id, companyId]);
