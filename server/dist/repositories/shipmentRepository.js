...
export class ShipmentRepository {
    applyScope(conditions, values, scope, tableAlias = '') {
        ...
        if (scope?.financeAgentScope && scope.agentId && scope.userId) {
            ...
            const destClause = destinationHints.length > 0
                ? (() => {
        ...
        if (scope?.agentId) {
            ...
            const destinationHints = [scope.agentArea, scope.agentCity, scope.agentGovernorate]
                .filter((v) => Boolean(v && v.trim().length))
                .map((v) => v.toLowerCase().trim());
            const orParts = [`${prefix}agent_id = ${agentParam}`];
            if (scope.branchId) {
                values.push(scope.branchId);
    ...
    async existsInCompany(id, companyId) {
        const values = [id];
        ...
        const result = await pool.query(`select 1 from shipments where ${conditions.join(' and ')} limit 1`, values);
        return (result.rowCount ?? 0) > 0;
    }
    async createWithClient(client, input) {
    ...
    async update(id, payload) {
        ...
        try {
            await client.query('begin');
            const previous = await client.query('select status from shipments where id = $1 and deleted_at is null', [id]);
    ...
    async remove(id, scope) {
        ...
        const result = await pool.query(`update shipments set deleted_at = now() where ${conditions.join(' and ')} returning id`, values);
        return Boolean(result.rowCount);
    }
    async transitionStatusCore(client, input) {
        const where = ['id = $1', 'deleted_at is null'];
        const values = [input.shipmentId];
    ...
    async listStatusHistory(shipmentId, scope) {
        const conditions = ['h.shipment_id = $1'];
        const values = [shipmentId];
        this.applyScope(conditions, values, scope, 's');
        const result = await pool.query(`
      select h.*
