...
export class SalaryRepository {
    // ── Salary Records ──────────────────────────────────────────────────────────
    async listSalaries(companyId, filters) {
    ...
    async getSalaryById(id, companyId) {
        const result = await pool.query(`
    ...
    async updateSalary(id, companyId, input) {
    ...
    async deleteSalary(id, companyId) {
        const result = await pool.query(`update salary_records set deleted_at = now() where id = $1 and company_id = $2 and deleted_at is null returning id`, [id, companyId]);
        return (result.rowCount ?? 0) > 0;
    }
    // ── Advances ────────────────────────────────────────────────────────────────
    async listAdvances(companyId, filters) {
    ...
    async getAdvanceById(id, companyId) {
        const result = await pool.query(`
    ...
    async updateAdvance(id, companyId, input) {
        const result = await pool.query(`
    ...
    async deleteAdvance(id, companyId) {
        const result = await pool.query(`update employee_advances set deleted_at = now() where id = $1 and company_id = $2 and deleted_at is null returning id`, [id, companyId]);
        return (result.rowCount ?? 0) > 0;
    }
    // ── Summary stats ────────────────────────────────────────────────────────────
    /** مجموع السلف المفتوحة (المتبقي) لموظف — يُدمج تلقائياً في خصومات الراتب عند الإنشاء ويُزامَن عند تغيّر السلف. */
    async sumOpenAdvanceBalance(companyId, employeeId) {
        const result = await pool.query(`
     ...
     */
    async adjustPendingSalaryDeductions(companyId, employeeId, periodYear, periodMonth, delta) {
    ...
    async getPendingSalaryCurrencyForPeriod(companyId, employeeId, periodYear, periodMonth) {
        const result = await pool.query(`
      select currency
      from salary_records
      where company_id = $1
    ...
    async listSalaryAdvanceDeductions(companyId, salaryRecordId) {
        const result = await pool.query(`
      select
        sad.*,
        ea.advance_date,
    ...
    async paySalary(id, companyId, input, financeRepository) {
        ...
        `, [id, companyId]);
            ...
            if (salary.payment_status !== 'pending')
                throw new HttpError(400, 'يمكن دفع الرواتب المعلقة فقط.');
        ...
        `, [input.cashboxId, companyId]);
            ...
            const existingDeductions = await client.query(`select count(*)::int as count from salary_advance_deductions where salary_record_id = $1::uuid`, [id]);
