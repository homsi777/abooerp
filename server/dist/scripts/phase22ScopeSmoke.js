...
async function runScopeSmoke() {
    ...
    try {
        ...
        const srs = await request(baseUrl, '/api/v1/senders-receivers');
        ...
        const shipmentA = await request(baseUrl, '/api/v1/shipments', {
            method: 'POST',
            body: JSON.stringify({
        ...
        const shipmentB = await request(baseUrl, '/api/v1/shipments', {
            method: 'POST',
            body: JSON.stringify({
        ...
        const shipmentsScopedA = await request(baseUrl, '/api/v1/shipments', {
            scope: { branchId: branchA },
        });
        ensure(shipmentsScopedA.status === 200, 'Scoped shipment list failed for branch A');
        ensure(shipmentsScopedA.body.data?.some((s) => s.id === shipmentAId) === true, 'Branch A scoped shipments missing own shipment');
        ensure(shipmentsScopedA.body.data?.some((s) => s.id === shipmentBId) === false, 'Branch A scoped shipments leaked branch B shipment');
        ...
        const manifestsA = await request(baseUrl, '/api/v1/manifests', { scope: { branchId: branchA } });
        ensure(manifestsA.status === 200, 'Scoped manifest list failed');
        const deliveriesA = await request(baseUrl, '/api/v1/deliveries', { scope: { branchId: branchA } });
