...
async function runPhase313ResetProtectionAuditSmoke() {
    ...
    try {
        ...
        const metricsRes = await request(baseUrl, '/api/v1/party-statements/dashboard-cache-metrics', { auth: { userId: adminUserId } });
        ...
        if (!metrics.resetControl.enabled) {
            const resetDisabledRes = await request(baseUrl, '/api/v1/party-statements/dashboard-cache-reset', {
                method: 'POST',
            ...
            const withoutConfirm = await request(baseUrl, '/api/v1/party-statements/dashboard-cache-reset', {
                method: 'POST',
                auth: { userId: adminUserId },
            ...
            const withConfirm = await request(baseUrl, '/api/v1/party-statements/dashboard-cache-reset', {
                method: 'POST',
                auth: { userId: adminUserId },
                body: JSON.stringify({ resetCache: true, resetMetrics: true, confirm: true }),
            });
            ...
            const resetRes = await request(baseUrl, '/api/v1/party-statements/dashboard-cache-reset', {
                method: 'POST',
                auth: { userId: adminUserId },
        ...
        const auditRes = await request(baseUrl, '/api/v1/party-statements/dashboard-cache-reset-audit?limit=5', { auth: { userId: adminUserId } });
