...
async function run() {
    ...
    const branch = await pool.query(`select id from branches where company_id = $1 order by created_at limit 1`, [companyId]);
    ensure(Boolean(branch.rowCount), 'No branch');
    const branchId = branch.rows[0].id;
    const srs = await pool.query(`select id from senders_receivers limit 2`);
    ensure((srs.rowCount ?? 0) >= 1, 'Need sender/receiver');
    const senderId = srs.rows[0].id;
    const receiverId = srs.rows[1]?.id ?? senderId;
    await pool.query(`delete from shipments where company_id = $1 and shipment_no like 'TEST-RECON-%'`, [companyId]);
    ...
    for (const dest of DESTINATIONS) {
        ...
        if (dest.governorate === 'الرقة') {
            ...
            const expectedNet = money(dest.transferFee + dest.hawalaAmount + dest.transferServiceFee - expectedCommission);
