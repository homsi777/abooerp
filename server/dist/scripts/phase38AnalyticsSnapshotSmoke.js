...
async function runPhase38AnalyticsSnapshotSmoke() {
    ...
    try {
        ...
        const party1 = await request(baseUrl, '/api/v1/senders-receivers', {
            method: 'POST',
            body: JSON.stringify({
        ...
        const party2 = await request(baseUrl, '/api/v1/senders-receivers', {
            method: 'POST',
            body: JSON.stringify({
        ...
        const receiptPartyA = await request(baseUrl, '/api/v1/receipt-vouchers', {
            method: 'POST',
        ...
        const paymentPartyA = await request(baseUrl, '/api/v1/payment-vouchers', {
            method: 'POST',
            auth: { userId: adminUserId },
            body: JSON.stringify({
        ...
        const receiptPartyB = await request(baseUrl, '/api/v1/receipt-vouchers', {
            method: 'POST',
            auth: { userId: adminUserId },
            body: JSON.stringify({
        ...
        const analyticsRes = await request(baseUrl, `/api/v1/party-statements/analytics?partyType=sender_receiver&fromAt=${encodeURIComponent(fromAt)}&topN=2`, { auth: { userId: adminUserId } });
