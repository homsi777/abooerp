...
const shipmentService = new ShipmentService(shipmentRepository, new InventoryService(), shipmentFinancialPostingService, new TransfersService(new TransfersRepository(pool), financeRepository), agentRepository);
...
const company = await pool.query('select company_id as id from branches where id = $1', [branchId]);
const companyId = company.rows[0]?.id;
if (!companyId) {
    throw new Error(`Branch not found: ${branchId}`);
}
const result = await dailyLedgerService.postPendingShipments({ companyId, branchId, userId: undefined }, { branchId, ledgerDate, lineLabel }, []);
