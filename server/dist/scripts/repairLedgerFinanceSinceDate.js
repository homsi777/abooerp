...
function buildPostingServices() {
    ...
    const shipmentService = new ShipmentService(shipmentRepo, new InventoryService(), financialPosting, new TransfersService(new TransfersRepository(pool), financeRepo), agentRepo);
    const postingService = new DailyLedgerShipmentPostingService(new DailyLedgerRepository(), shipmentService, agentRepo, financialPosting);
...
async function syncLedgerAmountsToShipments(fromDate) {
...
async function repairUnpostedFinancials(companyId, financialPosting) {
    ...
    for (const row of rows.rows) {
        ...
        try {
            ...
            failed += 1;
            console.error(`[financial] failed ${row.shipment_id}:`, error instanceof Error ? error.message : error);
        }
        finally {
            client.release();
        }
...
async function repairEffectiveDateUpdates(shipmentId, ledgerDate) {
    if (!(await columnExists('shipments', 'effective_date')))
        return;
    await pool.query(`update shipments set effective_date = coalesce($2::date, effective_date) where id = $1`, [shipmentId, ledgerDate]);
}
async function postPendingLedgerRows(companyId, postingService) {
    ...
    `, [companyId, FROM_DATE]);
    const branches = await pool.query(`select id from branches where company_id = $1`, [companyId]);
    ...
    for (const session of sessions.rows) {
        ...
        if (count <= 0)
            continue;
        if (DRY_RUN) {
            console.log(`[dry-run] would post ${count} rows for ${session.ledger_date} / ${session.line_label}`);
            continue;
        }
        const result = await postingService.postPendingShipments({ companyId, branchId: session.branch_id }, {
        ...
        if (result.posted.length || result.errors.length) {
            console.log(`[ledger] ${session.ledger_date} / ${session.line_label}: posted=${result.posted.length} errors=${result.errors.length}`);
...
async function main() {
    ...
    const company = await pool.query(`select id, name from companies order by created_at limit 1`);
    if (!company.rows[0])
