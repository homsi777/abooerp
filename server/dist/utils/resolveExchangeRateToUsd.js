 ...
 */
export async function resolveExchangeRateToUsd(exchangeRepo, companyId, currencyCode, effectiveDateIso) {
    ...
    if (byDate && byDate.rate > 0)
        return Number(byDate.rate);
    const latest = await exchangeRepo.getLatestRateByCode(code, companyId);
    if (latest && latest.rate > 0)
        return Number(latest.rate);
    throw new HttpError(400, `لا يوجد سعر صرف مسجّل للعملة ${code}. أضف سعراً في الإعدادات ← العملات وأسعار الصرف.`);
}
