...
function resolveRendererEntry() {
    ...
    return app.isPackaged ? { mode: 'file', value: prodFile } : { mode: 'url', value: devUrl };
}
...
async function runElectronRuntimeSmoke() {
    ...
    try {
        const probe = await Promise.race([
            ...
            new Promise((resolve) => setTimeout(() => resolve({ callable: false, available: false, message: 'timeout' }), 4000)),
        ]);
    ...
    const passed = checks?.hasPrinterRuntime &&
...
function spawnBundledServer() {
    ...
    const logLine = (line) => {
        try {
            appendFileSync(serverLogFile, `${new Date().toISOString()} ${line}\n`);
        }
        catch { }
...
function isPortOpen(port, host, timeoutMs = 400) {
...
function migrateLocalPackagedServerFlagFromLegacyEnv() {
    try {
        ...
        if (!existsSync(env) || existsSync(flag))
            return;
...
async function waitForServer(splash) {
    ...
    for (let i = 0; i < maxAttempts; i++) {
        ...
        if (i === 3)
            setSplashStatus(splash, 'جاري الاتصال بقاعدة البيانات...');
        if (i === 8)
            setSplashStatus(splash, 'جاري تهيئة الجداول...');
        if (i === 12)
            setSplashStatus(splash, 'يستغرق أطول من المعتاد...');
...
async function waitForRemoteHealth(splash, healthUrl) {
    ...
    for (let i = 0; i < maxAttempts; i++) {
        ...
        if (i === 4)
            setSplashStatus(splash, 'تحقق من IP الرئيسي والشبكة...');
        if (i === 12)
            setSplashStatus(splash, 'لا يزال الاتصال غير متاح...');
        await new Promise(r => setTimeout(r, 1000));
    }
...
async function bootstrapDesktopRuntime() {
    ...
    if (app.isPackaged) {
        ...
        try {
            ...
            const lanClient = cfg.backendResolutionMode === 'manual_lan' || cfg.backendResolutionMode === 'auto_lan';
    ...
    app.whenReady().then(async () => {
        app.on('web-contents-created', (_event, contents) => {
            contents.on('will-navigate', (event, navigationUrl) => {
                const isLocal = navigationUrl.startsWith('file://') ||
