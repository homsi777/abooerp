...
import type { ReceiptVoucher, Customer } from '../types';
...
export default function ReceiptVoucher() {
  ...
  const [formData, setFormData] = useState<Partial<ReceiptVoucher>>({
    voucherNo: '', date: new Date().toISOString().split('T')[0], customerId: 0, customerName: '', amount: 0, currency: 'USD', paymentMethod: 'cash', bankName: '', chequeNo: '', description: ''
  });
  ...
  const handleNew = () => {
    ...
    setFormData({ voucherNo: '', date: new Date().toISOString().split('T')[0], customerId: 0, customerName: '', amount: 0, paymentMethod: 'cash', bankName: '', chequeNo: '', description: '' });
    setIsEditing(true);
  };
  ...
  const handleSave = async () => {
    try {
      await phase3FinanceGateway.receiptVouchers.create({
        voucherNo: formData.voucherNo || `RV-FE-${Date.now()}`,
        status: 'draft',
      ...
      <div className="flex-1 flex gap-4 overflow-hidden">
        ...
        {isEditing && (
          <div className="w-96 card overflow-auto">
            <div className="card-header">{selectedVoucher ? 'تعديل سند قبض' : 'سند قبض جديد'}</div>
            <div className="space-y-3">
              ...
              <div className="form-group">
                ...
                <select className="form-select w-full" value={formData.customerId || ''} onChange={(e) => handleCustomerChange(Number(e.target.value))}>
              ...
              <div className="flex gap-2 pt-2">
                ...
                <button onClick={handlePrint} className="toolbar-btn">معاينة</button>
              </div>
            </div>
          </div>
        )}
