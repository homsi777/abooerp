...
async function spawnBundledServer(): Promise<void> {
...
async function waitForLocalSnapshot(splash: BrowserWindow): Promise<void> {
  ...
  for (let attempt = 0; attempt < 45; attempt++) {
    ...
    if (attempt === 14) setSplashStatus(splash, 'ما زالت مزامنة التهيئة جارية...');
  ...
  if (app.isPackaged) {
    ...
    try {
      ...
      if (existsSync(getLocalPackagedServerFlagPath())) {
        try {
          const postgres = await ensureLocalPostgresReady();
          if (!postgres.ready) throw new Error(postgres.reason || 'LOCAL_POSTGRES_NOT_READY');
          await spawnBundledServer();
        } catch (se) {
