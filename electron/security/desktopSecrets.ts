...
export type DesktopSecrets={postgresPassword:string;postgresUser?:string;centralSyncDeviceId?:string;centralSyncDeviceToken?:string;backupKey:string};
...
export function saveDesktopSecrets(secrets:DesktopSecrets){
...
export function loadDesktopSecrets():DesktopSecrets|null{
  if(existsSync(secretPath())){
    if(!safeStorage.isEncryptionAvailable())throw new Error('Windows secure storage is not available.');
    const parsed=JSON.parse(safeStorage.decryptString(readFileSync(secretPath()))) as DesktopSecrets;
    if(!parsed.backupKey){parsed.backupKey=randomBytes(32).toString('base64');saveDesktopSecrets(parsed)}
    return parsed;
  }
  ...
  if(!postgresPassword&&existsSync(legacyEnvPath())){
    const legacy=parseEnv(readFileSync(legacyEnvPath(),'utf8'));postgresPassword=legacy.PGPASSWORD||'';
    centralSyncDeviceToken=centralSyncDeviceToken||legacy.CENTRAL_SYNC_DEVICE_TOKEN||undefined;
  }
  if(!postgresPassword)return null;
  const secrets={postgresPassword,centralSyncDeviceToken,backupKey:randomBytes(32).toString('base64')};saveDesktopSecrets(secrets);scrubLegacySecrets();return secrets;
}
