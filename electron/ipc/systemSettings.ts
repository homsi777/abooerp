...
async function requestWithOptionalAuth(path: string, init?: RequestInit) {
...
export function registerSystemSettingsIpc() {
  ...
  ipcMain.handle(CHANNEL_SYSTEM_LIST, async (_event, payload?: { authToken?: string; branchId?: string | null }) => {
  ...
  ipcMain.handle(CHANNEL_SYSTEM_GET, async (_event, payload: { key: string; authToken?: string; branchId?: string | null }) => {
  ...
  ipcMain.handle(CHANNEL_SYSTEM_SET, async (_event, payload: { key: string; value: unknown; authToken: string; branchId?: string | null }) => {
    const response = await requestWithOptionalAuth(`/system-settings/${encodeURIComponent(payload.key)}`, {
      method: 'PUT',
      headers: {
