...
export function registerDesktopSetupIpc() {
  ...
  ipcMain.handle('desktop-setup:configure-postgres', async (_event, payload: { password?: unknown }) => {
    ...
    try {
      await admin.connect();
      const settings = await admin.query<{ listen_addresses: string }>(`select current_setting('listen_addresses') listen_addresses`);
      ...
      if (!existing.rowCount) {
        ...
        if (!/^almiya_device_[a-f0-9]{12}$/.test(role)) return { success: false, error: 'DEVICE_DATABASE_ROLE_INVALID' };
        runtimePassword = randomBytes(32).toString('base64url');
        const quotedPassword = (await admin.query<{ value: string }>(`select quote_literal($1) value`, [runtimePassword])).rows[0].value;
        const roleExists = await admin.query(`select 1 from pg_roles where rolname=$1`, [role]);
        if (roleExists.rowCount) await admin.query(`alter role "${role}" login createdb password ${quotedPassword}`);
        else await admin.query(`create role "${role}" login createdb password ${quotedPassword}`);
  ...
  ipcMain.handle('desktop-setup:list-central-branches', async () => {
    try {
      ...
      if (!response.ok || !Array.isArray(body?.data)) return { success: false, error: 'CENTRAL_BRANCHES_UNAVAILABLE', branches: [] };
      return { success: true, branches: body.data.map((branch: any) => ({ id: String(branch.id), code: String(branch.code ?? ''), name: String(branch.name ?? '') })) };
    } catch {
      return { success: false, error: 'CENTRAL_BRANCHES_UNAVAILABLE', branches: [] };
    }
  });

  ipcMain.handle('desktop-setup:activate-sync', async (_event, payload: { username?: unknown; password?: unknown; branchId?: unknown }) => {
    const username = String(payload?.username ?? '').trim();
    const password = String(payload?.password ?? '');
    const branchId = String(payload?.branchId ?? '');
    if (!username || !password || !/^[0-9a-f-]{36}$/i.test(branchId)) return { success: false, error: 'CENTRAL_CREDENTIALS_REQUIRED' };
    try {
      const runtime = await loadRuntimeConfig();
      const base = String(runtime.centralSyncApiBaseUrl || '').replace(/\/$/, '');
      const machineId = await resolveMachineId();
      ...
      const registrationBody = await registration.json().catch(() => null) as any;
      if (registration.status === 202 || String(registrationBody?.error ?? '').includes('PENDING')) return { success: false, error: 'DEVICE_PENDING_APPROVAL' };
      if (!registration.ok) return { success: false, error: String(registrationBody?.error ?? 'DEVICE_REGISTRATION_FAILED') };

      const login = await fetch(`${base}/auth/login`, {
        method: 'POST', headers: commonHeaders, signal: AbortSignal.timeout(12000),
        body: JSON.stringify({ username, password, branchId }),
      });
      const loginBody = await login.json().catch(() => null) as any;
      const accessToken = String(loginBody?.data?.session?.accessToken ?? '');
      if (!login.ok || !accessToken) return { success: false, error: 'CENTRAL_LOGIN_FAILED' };
      ...
      const activationBody = await activation.json().catch(() => null) as any;
      if (activation.status === 202 || String(activationBody?.error ?? '').includes('PENDING')) return { success: false, error: 'SYNC_DEVICE_PENDING_APPROVAL' };
      const deviceId = String(activationBody?.data?.deviceId ?? '');
      const deviceToken = String(activationBody?.data?.deviceToken ?? '');
      if (!activation.ok || !deviceId || !deviceToken) return { success: false, error: String(activationBody?.error ?? 'SYNC_ACTIVATION_FAILED') };
      const secrets = loadDesktopSecrets();
      if (!secrets) return { success: false, error: 'POSTGRES_CREDENTIALS_REQUIRED' };
