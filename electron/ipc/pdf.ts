...
async function generatePdfBuffer(html: string, landscape: boolean) {
...
export function registerPdfIpc() {
  ...
  ipcMain.handle(CHANNEL_PDF_EXPORT, async (_event, rawPayload: unknown) => {
    const payload = exportPdfSchema.parse(rawPayload);
    const host = resolveHostWindow();
    if (!host) {
      return { saved: false as const, filePath: null as string | null, message: 'No active Electron window.' };
    }

    const picked = await dialog.showSaveDialog(host, {
      title: payload.title,
      defaultPath: path.join(
        path.resolve(process.env.USERPROFILE || process.cwd(), 'Downloads'),
        payload.defaultFileName.endsWith('.pdf') ? payload.defaultFileName : `${payload.defaultFileName}.pdf`,
      ),
      filters: [{ name: 'PDF', extensions: ['pdf'] }],
    });

    if (picked.canceled || !picked.filePath) {
      return { saved: false as const, filePath: null as string | null, message: 'cancelled' };
    }

    const buffer = await generatePdfBuffer(payload.html, payload.landscape);
    await fs.writeFile(picked.filePath, buffer);
    return { saved: true as const, filePath: picked.filePath, message: 'saved' };
  });
}
