...
function getLocalLanAddressesElectron(): string[] {
...
type RuntimeMode = 'development' | 'local_production' | 'lan_node';
type BackendResolutionMode = 'localhost' | 'manual_lan' | 'auto_lan';
...
async function readJsonConfig(filePath: string): Promise<RuntimeConfig | null> {
  try {
...
function getDefaultRuntimeConfig(): RuntimeConfig {
  return {
    ...
    manualLanHost: '',
    backendPort: 4010,
    schemaVersion: '116_desktop_company_finance_scope_sync',
...
function buildApiUrl(host: string, port: number) {
  return `http://${host}:${port}/api/v1`;
}

async function isHealthyApiBase(apiBaseUrl: string): Promise<boolean> {
  try {
...
async function autoDetectLanApiBase(port: number): Promise<string | null> {
...
async function resolveApiBaseUrl(config: RuntimeConfig): Promise<string> {
...
function normalizeRuntimeMode(config: RuntimeConfig): RuntimeMode {
...
const deviceNameBySessionKey = new Map<string, string>();
let runtimeConfigSnapshot: { config: RuntimeConfig; at: number } | null = null;
let loadRuntimeConfigInFlight: Promise<RuntimeConfig> | null = null;

function deviceNameCacheKey(apiBase: string, accessToken: string) {
  return `${apiBase}\n${accessToken}`;
}
...
async function resolveDeviceNameFromSystemSettings(config: RuntimeConfig): Promise<string> {
...
async function readStoredSessionState(): Promise<StoredSessionState> {
  try {
...
async function writeStoredSessionState(next: StoredSessionState): Promise<void> {
...
async function writeStoredSessionToken(token: string | null): Promise<void> {
...
async function writeStoredActiveBranch(branchId: string | null): Promise<void> {
  const current = await readStoredSessionState();
  await writeStoredSessionState({
    ...current,
...
async function readPersistedMachineId(): Promise<string | null> {
  try {
...
export function registerRuntimeConfigIpc() {
  ...
  ipcMain.handle(CHANNEL_SET_SESSION_TOKEN, async (_event, token: string | null) => {
  ...
  ipcMain.handle(CHANNEL_SET_ACTIVE_BRANCH, async (_event, branchId: string | null) => {
  ...
  ipcMain.handle('runtime:test-lan-server', async (_event, ip: string, port: number) => {
    try {
